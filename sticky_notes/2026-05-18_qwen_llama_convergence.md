...
## Update 2026-09-13 — Cross-Resolution Cluster Reconciliation

AA-12 follow-up 3 adds direct partition evidence over the identical 275-role inventory without aligning activation bases or using trait semantics. Across all Qwen/LLaMA/Gemma K=4–10 combinations, Qwen K=6/LLaMA K=9/Gemma K=6 maximizes mean pairwise AMI at 0.644895 and remains best after requiring frozen refit ARI at least 0.70. A 5,000-draw null preserving each model's exact cross-K partition structure gives search-adjusted p=0.000200, and eight neighboring triples lie within 0.02 of the maximum. Direct role-overlap and neighboring-K persistence support four broad all-model candidate families plus a recurring developmental outlier family.

This is stronger evidence that cross-model recurrence extends to discrete role-family organization, not just aligned continuous coordinates. It still does not identify training-data lineage, distillation, corpus composition, or shared architectural constraints as the cause: the same English role inventory is itself a major shared input. The sticky note therefore remains a hypothesis/framing note rather than being marked addressed.
