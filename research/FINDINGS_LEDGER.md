...
## AA-19 · independent SAPA test of model-consensus axes (2026-09-17)

Observed: the AA-18 inputs and external 23,679-row SAPA file match frozen hashes; the 45-direct human proxy correlation matrix rederives from raw psychological items to maximum error 5×10⁻⁹. The primary future-safe, unique-source direct bridge has 41 proxies. Independently fitted human oblique factors retain 5 under held-out one-standard-error selection, while parallel analysis and MAP each indicate 6; post-freeze split sensitivity selects 5 in 7/8 sets. Trait-covariance upper-triangle correlations with the human matrix are 0.437/0.611/0.455 for Qwen/Llama/Gemma and 0.531 for their consensus (500-label-permutation p=0.002). Under frozen bridge, bootstrap, and null criteria: C3 is class A in the primary five-factor fit, C1 class C (human combination), C2 class B (partial), and C4/C5 class D (presently untestable from direct safe proxies). Overall decision 2: partial human convergence. C3's best individual human-factor congruence falls from 0.744 to 0.675 under six factors, so its single-axis match is factor-count sensitive despite stable subspace projection. Its human projection correlates +0.814 with official IPIP100 Agreeableness in a descriptive, shared-item-sensitive comparison. No HiFWB outcome entered any fit or selection; this is structural evidence, not causal traits or human/model equivalence. Source: `research/outputs/aa19_human_consensus_factor_validation/three_model_human_validation_report.md`.

## AA-18 · three-model consensus trait structure (2026-09-17)

Observed: the exact AA-17 Qwen, Llama, and Gemma signed-cosine matrices have identical 275 persona and 240 trait orders and pass the reconstruction gate. Of 240 named traits, 220 have all-three minimum pairwise persona-profile Pearson r≥0.50; 9 are pairwise-only by the frozen rule. Regularized MAXVAR-GCCA yields 13 provisional held-out all-three score directions, but only C1–C5 meet the stricter axis-level score, trait-loading congruence, bootstrap, split-half, null, and regularization criteria. An AJIVE-like rank-20 in-sample row-subspace validation finds joint rank 19 and standardized joint variance 0.980/0.969/0.972 in Qwen/Llama/Gemma; this is shared persona-span evidence, not 19 interpretable factors. Decision A: five conservative model-only consensus axes. AA-17's grounded/spiritual factor spreads chiefly over C1/C2; the Llama/Gemma cooperative-optimism factor has an all-three shared portion in C1/C3. Direct AA-16 bridge labels cover only 13–20% of absolute mass on the five axes; human comparison is conditionally ready only for restricted, duplicate-aware sensitivity work, especially given sparse C4/C5 salient labels. Original response-level extraction remains unverified. No causal trait, human equivalence, wellbeing, inference, or paid-compute claim follows. Source: `research/outputs/aa18_three_model_consensus_trait_structure/three_model_consensus_trait_report.md`.

## AA-17 · three-model persona trait factor analysis (2026-09-17)

Observed: the three saved 275-persona × 240-trait signed cosine matrices pass the matrix phase gate with identical persona and trait order, no missing or degenerate columns, and severely ill-conditioned ordinary trait correlations (condition numbers 2.6e9–4.5e10). Within-model Ledoit–Wolf shrinkage, iterative principal-axis factoring, and oblimin rotation retain 5 Qwen, 6 Llama, and 6 Gemma factors by 100-draw marginal-preserving parallel analysis. MAP reaches its tested boundary of 15, and heldout Gaussian likelihood improves across the candidate range, so the count is descriptive and not uniquely established. Twenty persona bootstraps and ten repeated split halves support all five Qwen axes individually, only the first three Llama axes, and Gemma F1–F4/F6 under the frozen thresholds; Llama F4–F6 and Gemma F5 remain individually unresolved despite stable within-model subspaces. The clearest all-model match is Qwen F3 / Llama F2 / Gemma F6 (absolute Tucker congruence 0.919–0.930). Decision C: partially shared factors with substantial model-specific structure. This is same-space activation-cosine co-expression, not causal or human personality measurement. The 45-direct/74-total SAPA bridge is used only for post-fit readiness coverage, with no HiFWB input. Start at `research/outputs/aa17_three_model_trait_factor/three_model_trait_factor_report.md`.

## AA-16 Stage 1 · human HiFWB association coverage (2026-09-17)

Observed: The frozen 240-label model-trait vocabulary has 45 direct, 29 close, and 166 absent SAPA proxy mappings. Exact outcome-item overlap excludes five mapped labels from the composite, leaving 42 direct and 69 direct-plus-close observed composite associations, corresponding to 65 unique exact item-set sources; four exact duplicate groups involve eight labels. Optimistic (r=+0.679, N=969) and pessimistic (r=-0.695, N=426) are the strongest positive and negative directly supported nonoverlapping composite proxies. All 3,360 trait–outcome rows are preserved with explicit missing values where unavailable. Interpretation: limited mapping coverage and repeated human sources permit a descriptive human-evidence profile, not a full-vocabulary wellbeing landscape or any claim about model representations. Hypothesis: indicator domains may differ in their supported trait associations. Source: `research/outputs/aa16_sapa_hifwb_trait_profile/stage1_findings.md`.
...
### AA-24 Model-Consensus-Axis HiFWB Transport (2026-09-17)

- The final planned outcome transported the independently frozen AA-18 consensus axes through the human HiFWB bridge, restricting the primary analysis to C1–C3 because AA-19 found C4/C5 bridge-limited. On the exact AA-20 common sample (N=2,859; test N=564), consensus C1–C3 alone achieve held-out R²=.178, Big Five=.494, and combined=.498.
- The consensus-axis increment is ΔR²=+.005 with paired test-resample 95% interval [-.007,+.017]; the human incremental gate remains closed. This reproduces AA-20 rather than adding evidence for incremental validity.
- Full-sample human association weights yield relative consensus-axis wellbeing hypotheses for all 275 Qwen, Llama, and Gemma personas. Their correlations with AA-21 Big Five rankings are .413/.541/.423 (Qwen/Llama/Gemma Pearson) and with AA-23 residual rankings .526/.376/.525; train+validation versus full-sample rankings exceed .997 Spearman.
- Interpretation: the model-consensus transport is a distinct, stable ranking hypothesis, not a validated wellbeing measure. C4/C5 remain untested, and no causal, human-equivalence, or model-subjective-wellbeing claim follows. Source: `research/outputs/aa24_model_consensus_hifwb_projection/aa24_model_consensus_hifwb_report.md`.

### AA-25 Stage 1 Intervention-to-Big-Five Transport (2026-09-17)

- Mapped the frozen 240 model traits into Big Five-like domains using the existing external-taxonomy construction, reversing Neuroticism to emotional stability. The resulting model-derived domain scores align strongly with existing human-anchored persona scores for Agreeableness, Conscientiousness, Extraversion, and emotional stability across all three models; Openness is weaker but positive.
- Defined five literature-informed, direction-only intervention probes: emotional-stability therapy, social activation, behavioral activation, broad volitional growth, and openness training. HiFWB changes were calculated from the frozen human Big Five coefficients using a one-standard-deviation sensitivity convention, not a claimed intervention dose.
- Interpretation: the static model trait spaces can represent intervention-relevant Big Five directions, so they are usable for a next-stage model-response or activation experiment. This stage does not show that models change after an intervention or that their predicted changes match humans. No model inference, RunPod, paid compute, or respondent-row export occurred.
