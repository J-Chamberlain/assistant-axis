...
The availability-only greedy nested sequence starts at the highest-coverage item and adds the item maximizing strict complete cases on the expanded panel, with deterministic marginal-N and canonical-order tie-breaks. Strict completeness falls from 6,096 respondents at one item to 1,455 at two, 358 at three, 103 at four, 41 at five, 11 at ten, 6 at twenty, 3 at 39, and 2 at 64. A single respondent persists from panel size 137 through 294 before strict completeness reaches zero at 295; this long tail is an algorithmic consequence of optimizing an already tiny cohort, not evidence of a broadly usable profile panel. On the identical nested panels, relaxed 95%, 90%, and 80% counts reach zero at 310, 327, and 368 items.

Interpretation: the very steep early bend is a descriptive coverage constraint created by planned random-subset administration, not a psychological finding or a selected research threshold. The full aggregate frontier supports the next user decision about what dimensionality/sample-size region is worth considering. It does not decide the item panel, respondent cohort, complete-case/partial-profile method, or common human/model feature space. No respondent-level data were committed and no new scientific claim was created.
