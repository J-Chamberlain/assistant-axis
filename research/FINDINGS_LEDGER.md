...
### Qwen PC-Specificity Separates a Smaller Diagnostic Core From Broad Correlation Families (2026-09-12)

The completed AA-1 association inventory is preserved exactly, then refined with paired cross-PC bootstrap specificity over the same 275 Qwen roles. A PC-defining trait must satisfy the unchanged AA-1 salience rule and have its target PC be the largest absolute PC1-PC6 correlation in at least 95% of 2,000 fixed-seed role resamples. The 328 associated memberships contract to 202 PC-defining memberships; 70 of those have observed PC1-PC6 concentration at least 0.75. Associated/defining counts are PC1 positive 47/35 and negative 123/95, PC2 31/12 and 44/23, PC3 42/18 and 31/18, PC4 2/0 and 6/1, PC5 1/0 and 0/0, and PC6 0/0 and 1/0. Sensitivity totals are stable to the target-r grid but respond to dominance confidence: total defining memberships range from 202 at the 0.40 or 0.50 effect threshold plus 0.95 dominance to 199 at 0.60/0.95, and from 211 at 0.50/0.90 to 184 at 0.50/0.99.

PC1 remains broad but contracts meaningfully. `transparent`, `factual`, `analytical`, and `methodical` are highly concentrated PC1-positive traits, whereas `conscientious` is broad/cross-PC because PC1 r=0.693 is slightly smaller in magnitude than PC2 r=-0.699. PC3's strongest requested semantic anchors are genuinely specific: positive `callous`, `cynical`, and `pessimistic`, and negative `benevolent`, `nurturing`, `supportive`, and `altruistic` are highly concentrated; `agreeable`, `forgiving`, `collaborative`, and `tactful` are moderate; `empathetic`, `accommodating`, and `conciliatory` are broad/cross-PC. The all-trait Pareto leaders are `transparent`, `narrative`, `experiential`, `introverted`, `callous`, and the pair `benevolent`/`nurturing` for the six well-populated PC1-PC3 poles.

The exact indexed Qwen Affiliation visualization composite was recovered rather than inferred: equal-weight mean of within-Qwen midrank percentiles for `empathetic`, `agreeable`, and `altruistic`. It has PC3 r=-0.944857, largest off-target PC1 r=-0.110061, specificity margin 0.834796, ratio 8.584854, and PC1-PC6 concentration 0.978101. It is a reference only and did not determine thresholds.

The 48 frozen SAPA judgments are unchanged. Their explicitly listed covered traits account for 145/328 associated memberships (44.2%) and 93/202 defining memberships (46.0%). The human-measurement picture becomes qualitatively more focused rather than uniformly stronger: PC3's diagnostic core is well represented by existing Agreeableness/Compassion/Cooperation and antagonism-related proposals, while PC1 retains many unmapped defining traits and later PCs have little or no defining core. This is measurement availability, not human/model correspondence. Verification passes 29 checks; no respondent scoring/projection, cross-model comparison, AA-7 result, next-experiment selection, GPU, inference, or respondent-level data entered the analysis.

### Correlation-Defined Qwen Trait Families Have Uneven SAPA Measurement Coverage (2026-09-12)

Across the 275 saved Qwen/Qwen3-32B roles, all 240 activation-derived trait affinities were correlated with Qwen PC1-PC6. Under the rule frozen before semantic review—absolute Pearson r at least 0.50, BH-FDR q below 0.01 within each PC, and sign stability of at least 95% across 2,000 fixed-seed role bootstraps—the twelve signed families contain 328 PC-trait memberships spanning 238 unique traits. PC1 has 47 positive and 123 negative members; PC2 has 31/44; PC3 has 42/31; PC4 has 2/6; PC5 has 1/0; and PC6 has 0/1. Canonical PC1-PC3 coordinates reproduce to maximum absolute error `1.207027046312703e-06`.

The PC-blind mapping packet was frozen before human measurement judgments and contained neutral family IDs, the selected trait names/definitions, correlations, anchors, and member-derived descriptions only. Reviewing that packet against the full 126-construct SAPA library plus item fallback yielded 48 proposed counterparts: 6 strong, 12 partial, 26 facet/subcomponent, 2 item-only, and 2 no-match rows. At family level, four signed families have at least one strong SAPA match, five have partial matches, one has only facet/item coverage, and the two numerically empty poles have no counterpart. The inventory uses 37 unique construct/item candidates and 350 unique SAPA items; seven constructs and 144 items recur across more than one family, with maximum family reuse of 4 and 6 respectively.

Interpretation: PC1-negative's expressive, symbolic, imaginative family, both PC3 antagonism/care poles, and the PC5-positive divergent singleton have strong available SAPA counterparts. PC1-positive, both PC2 poles, and both PC4 poles are compound or heterogeneous and receive partial rather than single-construct matches; PC6-negative has only intellect/complexity components rather than a direct systems-thinking scale. These are semantic measurement-availability judgments, not evidence that humans and Qwen share a latent construct. No respondent was scored or projected, no Llama/Gemma or AA-7 result informed selection, and no next experiment was selected.

### Independent SAPA Review Supports a Broader Qwen-First Construct Bridge, With Nine Semantic Discordances (2026-09-12)

The exact separate Claude Opus 5 blinded review of all 78 frozen SAPA candidates agrees with the prior Codex decisions on 69 rows (88.46%; unweighted Cohen's kappa 0.785). Retained/nonretained agreement is 94.87% (kappa 0.642), direct-status agreement is 93.59% (kappa 0.867), and the descriptive consensus contains 44 direct, 21 close, 4 nonretained, and 9 discordant rows. The discordant traits are risk_taking, mercurial, zealous, mischievous, disorganized, cautious, independent, nonchalant, and melancholic; neither judgment set was altered. Claude was separate and geometry-blind as documented, but remains an Anthropic AI system, so this is independent semantic-review evidence rather than expert psychometric validation.

Primary-source checks resolve the eight reviewer flags without rewriting historical packets. `q_251` and `q_1483` have authoritative exact IPIP wording available for future derivative metadata; `q_1671` remains bracketed in both the SAPA raw source and SPI documentation and must be source-qualified. Five apparent direction discrepancies (`q_1758`, `q_566`, `q_463`, `q_1742`, `q_1624`) reflect trait-specific use of same/reverse item orientation rather than changes to the official SAPA scoring keys; future bridge metadata now makes each direction explicit.

The broad construct inventory contains 126 sourced human constructs across Big Five, BFAS, metatraits, EPQ, HEXACO, IPIP-NEO, MPQ, QB6, and SPI structures. A Qwen-only multi-evidence discovery procedure freezes 37 PC1-PC6 hypotheses: 4 strong candidates, 12 plausible partial candidates, and 21 facets/subcomponents; 36/37 have SAPA measurement content. PC1 is provisionally characterized by high Stability/low Plasticity, with Conscientiousness the strongest single domain plus Orderliness, Industriousness, Achievement Striving, Self-Discipline, and Dutifulness facets and a residual external-standard-accountability component. PC3 retains a strong negative-Agreeableness core, supplemented by low Honesty-Humility/Compassion/Politeness/Altruism and high Aggression/Machiavellianism; transgression, deception, technical, and outsider structure remains broader than Agreeableness alone. PC2 is compound. PC4-PC6 have partial/provisional rather than strong human analogues; PC6 uniquely requires an external Need-for-Cognitive-Closure measure.

Interpretation: the earlier 12-trait human structural filter answered whether narrow proxies are internally differentiated, so its redundancy penalty excluded broad Conscientiousness because the proxy overlapped `methodical`; that exclusion does not make Conscientiousness a poor PC1-level descriptor. A domain/aspect/facet hierarchy is more coherent for broad PCs than literal one-to-one matching against 240 model labels, but it remains a discovery hypothesis set. Planned SAPA administration still prevents complete construct profiles and requires a later pairwise/latent/full-information scoring design. No Llama/Gemma or AA-7 result was used for selection; no respondent projection or human/model correspondence test was performed.
...
Centroid precision improves substantially with N. Across eligible reference cells, median expected five-dimensional error is about 0.43-0.46 at N=20, 0.26-0.28 at N=50, and 0.17-0.18 at N=100. Joint N/bootstrap/split-half/heterogeneity criteria classify 9 of 151 reportable narrow occupations as strong and 18 as moderate/exploratory; 18 of 22 reportable broad SOC families are strong and 2 moderate. N=20 is therefore too noisy for confirmatory use, N=50 is a defensible exploratory floor only with empirical stability checks, and N=100 is a useful strong-centroid screen but not sufficient by itself.

Stable estimation does not imply occupational distinctiveness. The exact finite-sample narrow decomposition assigns 8.2%-12.0% of standardized domain variance to sampled occupation means, but subtracting their Taylor sampling variance reduces the survey-weighted sensitivity ICCs to 3.8%-7.8%. At the broad level the corrected ICCs are only 0.8%-3.2%. Fixed cross-validated Big Five classification is just 0.009-0.034 balanced-accuracy points above the multiclass baseline. Within-occupation personality heterogeneity overwhelmingly dominates, and broadening produces precise but semantically mixed, weakly differentiated groups.

Secretary/administrative assistant is a strong narrow future candidate; accountant/auditor, caregiver, and the `production workers, all other` artisan proxy are moderate. Counselor, programmer, engineer, therapist, psychologist, journalist, teacher/instructor, and actor have only broader stable substitutes with explicit semantic cost; lawyer has no stable reportable parent. Round-6/Round-12 occupational recurrence is mixed and more consistent for Conscientiousness than Agreeableness, with severe younger-cohort/weight/few-cell limitations. Interpretation: selected occupational aggregates can support later preregistered distribution comparisons, but occupation is not a strong human personality type or proxy. No model geometry or human-to-model projection was used, and no respondent-level data were committed.
...
### Externally Anchored Agreeableness Strongly Aligns With Negative Qwen PC3 (2026-09-11)

The Big Five mapping was frozen before geometry inspection from public-domain IPIP/IPIP-NEO facets and the AA-1 reviewed SAPA bridge. The primary strict construction uses 42 defensible domain indicators among reviewed `ACCEPT_DIRECT` links; a separately flagged extended construction adds 25 `ACCEPT_CLOSE` links, and an all-240-trait external-taxonomy review includes 109 high-confidence direct indicators while excluding 75 proxies and leaving 56 unmapped. The historical hand-predeclared composite is comparator-only. No persona coordinate, PC correlation, surface, cluster, screenshot, AA-4 result, or downstream performance was available during A/B/C mapping.

Under the frozen strict construction, Qwen Agreeableness versus PC3 has activation-direction cosine -0.847133, Pearson r=-0.881714, Spearman rho=-0.837526, single-domain R2=0.777420, joint-five standardized beta=-0.894047, in-sample partial R2=0.533524, and held-out partial R2=0.524122. Agreeableness is Qwen PC3's dominant strict Big Five correlate; the single-domain model leaves 22.258% variance. The result is stable across extended, external-expanded, and historical constructions (Pearson r=-0.894312/-0.827072/-0.907201), positive-only versus bipolar scoring, and leave-one-trait/facet omissions. It is therefore not driven solely by the `agreeable` label, although A4 Cooperation is the largest facet contributor.

Cross-model comparison rejects a universal PC3 identity. Llama Agreeableness-PC3 is weak (direction cosine -0.192846, r=-0.163861) and Llama Agreeableness instead aligns strongly with negative PC2; Gemma shows a moderate negative-PC3 relation (cosine -0.690086, r=-0.570857) plus a substantial PC1 association. Interpretation: negative Qwen PC3 has a strong externally anchored activation-derived Agreeableness component, while the broader cooperative-care/system-stabilizing versus disruptive/internal-objective interpretation retains residual structure. This is same-space model evidence, not independent human psychometric validation, construct equivalence, shared cross-model semantics, or behavioral realization.

### Human SAPA Responses Sharply Contract the Provisional Direct Trait Bridge (2026-09-11)

Applied a rubric frozen before covariance inspection to the 45 `ACCEPT_DIRECT` and 29 separately flagged `ACCEPT_CLOSE` SAPA links using only human item responses and official SAPA scoring keys. Pairwise-complete Pearson is primary under the randomized planned-administration design; Spearman is a sensitivity analysis. Twelve direct links meet moderate-or-better human-measurement support (9 high, 3 moderate): adventurous, altruistic, forgiving, grandiose, impulsive, manipulative, optimistic, pessimistic, traditional, innovative, introspective, and judgmental. Thirty are weak/redundant under the frozen discriminant/reuse rules and 3 single-item proxies are insufficient rather than empirically rejected. Internal coherence is usually good (38 strong, 3 moderate, 1 weak among multi-item/direct proxies), but reuse collapses many semantically distinct labels onto shared evidence: the 74-link set contains 4 exact/inverse item-set aliases, 34 shared-item-sensitive pairs, and 11 same-scale high-redundancy pairs.

The 45×45 direct human proxy matrix retains structure beyond a handful of broad factors but not 45 independent dimensions: parallel analysis retains 6 components, participation-ratio effective rank is 12.44, and entropy effective rank is 20.84. Human-side official IPIP100 Big Five domains account for a median descriptive multiple R² of 0.334, with 42/45 direct traits below 0.50. Adding all 29 close links raises participation-ratio rank only to 15.14 and supplies 5 additional moderate-support links; 24/29 are redundant/broad or insufficient.

Planned missingness blocks literal complete respondent profiles. Direct evidence uses 96 items, of which a respondent sees 10 at the median; all retained evidence uses 129, of which a respondent sees 13. No respondent sees either complete item set, although item-pair overlap is substantial (median 490 direct; 504 all retained), supporting group-level pairwise structure. Interpretation: a restricted 9-high + 3-moderate direct bridge merits independent expert review and a separate respondent-scoring methods study; the 45- or 74-link bridge should not be projected as a validated complete human profile. No model geometry or human-to-model projection was used, and no respondent-level data were committed.
...
### SAPA/NLSY97 Human-Data Feasibility Is Partial and Measurably Constrained (2026-09-11)

Completed `research/outputs/human_trait_dataset_feasibility/` using the official Harvard Dataverse SAPA V5 release and an official NLS Investigator public-use NLSY97 extraction. SAPA V5 verifies at 23,679 respondents, 696 psychological items, 92 administered source constructs, and 131 separate derived scoring keys. An all-240-trait auditable lexical/local-TF-IDF plus Codex review found 78 direct/near-direct, 49 close narrow, 41 broad-only, and 72 no-defensible-coverage model-trait links. The 127 category>=2 traits use 180 distinct accepted item anchors; these are feasibility judgments with exact evidence, not psychometrically validated equivalences. Planned subset administration is substantial: the median respondent answered 71/696 items and the overall empty item-cell fraction is 87.62%, so later scoring must treat the design as planned missingness rather than ordinary nonresponse.

The official NLSY97 extract contains all 8,984 cohort respondents and exact public-use item/occupation metadata. Round-12 TIPI is complete for 7,044 respondents and jointly observed with a valid same-wave current/current-most-recent 2002 Census occupation for 6,261; analogous Round-6 Goldberg counts are 4,851 and 4,060. Reusing the prior conservative role/SOC judgments yielded 98 translated persona-role rows: 2 strong (N>=100; one unique secretary/administrative-assistant cell), 5 moderate, 20 exploratory, and 71 too sparse. Attractive exact/close roles such as lawyer (17), programmer (13), journalist (9), writer (8), engineer (7), therapist (7), psychologist (3), and actor (0) remain sparse and were not post hoc broadened. Later employment, education, justice, health, family-income, marital-status, and conditional substance-use probes coexist for many primary-sample respondents, but no outcome association was analyzed.

Interpretation: the data justify an independent review and preregistration of a restricted SAPA construct bridge plus a small set of adequately populated occupation comparisons. They do not justify treating all 240 labels as human-measured traits, mapping respondents into Qwen space, assigning persona identities to respondents, or claiming model/human correspondence. No respondent-level data were committed and no GPU, RunPod, inference, activation extraction, or external model API was used.
