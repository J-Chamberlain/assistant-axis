...
| Qwen-first human-construct bridge | Preserve a separate blinded SAPA semantic review and freeze broad human-construct hypotheses for Qwen PC1-PC6 before any held-out validation. | Exact Claude Opus 5 review; frozen Codex review; primary SAPA/IPIP/SPI metadata; sourced 126-construct human inventory; Qwen-only PC1-PC6 signatures. | Agreement/discordance and metadata-quality audits; 48 construct candidates; 37 frozen domain/aspect/facet hypotheses; SAPA feasibility; future held-out-validation plan. | discovery/future-work hypothesis construction only; not cross-model validation, human/model equivalence, or respondent projection |
...
Source artifact: `research/outputs/qwen_pc_human_construct_bridge/qwen_pc_human_construct_bridge_report.md`; exact external review: `sapa_category3_blinded_review_judgments.csv`; frozen discovery set: `qwen_derived_human_construct_hypotheses_v1.csv`.

- Observed: the separate Claude Opus 5 review agrees exactly with the frozen Codex review on 69/78 rows (88.46%; unweighted kappa 0.785), with nine explicit discordances and 44 consensus-direct, 21 consensus-close, and 4 consensus-nonretain rows. This is separate AI semantic adjudication, not independent human/expert psychometric validation.
- Observed: all eight flagged wording/direction cases were checked against primary SAPA/IPIP/SPI sources. Historical frozen packets remain unchanged; future derivative metadata records exact wording where recoverable, source-qualifies bracketed `q_1671`, and makes five trait-specific reverse/same orientation uses explicit.
- Observed: a sourced human inventory contains 126 constructs across domain, aspect, facet, and narrow levels. The Qwen-only PC1-PC6 signature contains all 240 trait associations per PC, role extremes, stability/variance, Qwen-filtered Big Five associations, existing interpretations, AA-4 evidence where available, and the PC1 accountability intervention. Llama/Gemma and AA-7 results were firewalled from selection.
- Observed: 37 discovery hypotheses were frozen before the future validation plan: 4 strong broad candidates, 12 plausible partial candidates, and 21 facets/subcomponents. Thirty-six are SAPA-available; PC6 Need for Cognitive Closure requires another measurement source. Severe planned missingness still prevents literal complete profiles.
- Interpretation: the earlier 12-trait narrowing penalized narrow-proxy redundancy and therefore excluded broad Conscientiousness; it does not imply that Conscientiousness is a poor PC1-level descriptor. PC1 is provisionally high Stability/low Plasticity with Conscientiousness the strongest single domain and a residual external-standard-accountability component. PC3 has a strong negative-Agreeableness core plus antagonism/deception residuals. PC2 is compound, and PC4-PC6 have only partial/provisional human analogues.
- Boundary: all rows are `DISCOVERY_ONLY / NOT YET CROSS-MODEL VALIDATED`. The bridge does not show that the same latent constructs exist in humans and Qwen, does not project a respondent, and must not be used as Paper 1.5 model-geometry evidence. Independent human expert review is the next gate.
...
| OQ9 | Does an independent human expert retain the frozen Qwen-derived domain/aspect/facet hierarchy, can a planned-missingness measurement model recover defensible respondent scores, and can a preregistered held-out source test the hypotheses without tuning them? | A separate Claude review agrees on 69/78 semantic rows, but remains AI adjudication. The 37-row Qwen discovery set recovers broad Conscientiousness for PC1 and negative Agreeableness for PC3; 36 rows are SAPA-available, yet no complete respondent profile exists and no correspondence analysis has run. |
...
| SAPA/NLSY97 feasibility audit, occupational-centroid stability study, provisional/narrow bridge, human-only SAPA structure audit, and Qwen-derived human-construct bridge as Paper 1.5 model results | Use only to motivate carefully bounded future independent validation; the narrow audit contracts the direct bridge to 9 high + 3 moderate links, the broader Qwen bridge freezes discovery hypotheses, and AA-5 identifies selected stable cells, but none establishes construct equivalence, model correspondence, or a respondent-scoring solution, and none belongs in the model-geometry evidence chain. |
