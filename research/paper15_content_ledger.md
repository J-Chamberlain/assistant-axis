...
| Qwen trait sparsity and basis coverage | Determine how many real trait directions are needed and compare them with random real-trait subsets, isotropic 5,120-D directions, and fold-local persona-span directions. | Existing 275 x 240 Qwen cosine matrix; canonical PC targets/loadings; saved 5,120-D role/trait vectors; fixed editorial 15. | Repeated nested joint/PC-specific selection, permutation and sparse stability, redundancy groups, matched random bases, direct span coverage, role-norm audit. | mixed compact real-trait advantage plus high-k generic basis coverage; not independent psychometrics |
...
Source artifact: `research/outputs/qwen_trait_sparsity_prediction/qwen_trait_sparsity_report.md`; machine-readable summary: `validation_summary.json`; matched control table: `matched_basis_comparison.csv`; direct span audit: `trait_span_pc_coverage.csv`.

- Observed: source-code and numerical reconstruction confirm that canonical persona PCA mean-pools 275 stored role tensors and fits PCA directly to the resulting 275 x 5,120 activation-coordinate matrix. The 240 named traits are not PCA variables. Predictor features and PC targets are nevertheless algebraically dependent because both transform the same role activation vectors.
- Observed: the exact canonical 240-trait LOPO baseline reproduced at R2=0.999522/0.998811/0.999611 and normalized 3D RMSE=0.045378. Leakage-safe joint forward selection reached all-PC R2>=.90/.95/.98/.99 at k=3/3/4/6. Normalized-RMSE thresholds .25/.15/.10 and <=2x canonical full error were reached at k=4/6/12/14.
- Observed: selected 5/10/15 real traits reached normalized RMSE=0.176511/0.106880/0.087216. The fixed editorial 15 reached 0.165056, while 100 random real-trait subsets at k=15 had median 0.153307. Optimized 15 reduced error 47.2% relative to the fixed set and beat every random-real k=15 subset.
- Observed: at k=15, median isotropic-random and fold-local training-persona-span random-direction normalized RMSE was 0.737911 and 0.170184; optimized real traits beat all 100 banks in both controls. At k=240, fold-local persona-span random directions reached 0.051422 versus repeated full-real 0.047609, while ambient isotropic directions remained at 0.124511.
- Observed: the full 240-trait activation-space span contains squared fractions 0.987955/0.985303/0.981947 of PC1/PC2/PC3 loadings. Role-vector norms have CV=0.0285 but correlate with PC1/PC2 at Pearson r=0.582/-0.683, so cosine projections are close to scaled linear projections only with a material norm qualification.
- Observed: exact compact winners are redundant and PC-specific. There are 2,178 pairs with absolute profile correlation >=0.90. Joint robust signals include ethereal, cryptic, serene, structuralist, fatalistic, and poetic, but all have or compete with correlated substitutes. PC1 repeatedly selects transparent, PC2 introverted, and PC3 benevolent/nurturing; one universal compact label set is not supported.
- Interpretation boundary: the result favors a mixture. Real traits have a strong compact-k predictive advantage, while large generic persona-span banks nearly reconstruct the full result. This supports narrow trait-specific alignment beyond matched bases but shows that the near-ceiling 240-feature result is substantially generic same-space basis coverage. It does not establish causal psychological dimensions, a validated human trait ontology, or behavioral realization.
...
| Qwen optimized compact trait profile | `research/outputs/qwen_trait_sparsity_prediction/matched_basis_comparison.csv` | selected k=15 R2 0.998360/0.996759/0.997313; normalized RMSE 0.087216 | observed, provenance-coupled | Strong low-k real-direction advantage; fold-local persona-span directions nearly match the real bank at k=240, so the complete result remains largely basis coverage. |
...
| Qwen trait sparsity and matched generic-basis audit | Central qualification for the near-ceiling complete-profile result: compact selected real traits outperform generic directions, but high-k persona-span random bases nearly match the complete bank. |
