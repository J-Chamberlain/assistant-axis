...
## AA-20 human-only HiFWB association test of AA-19 consensus axes

Entry point: `research/outputs/aa20_consensus_axes_hifwb/aa20_consensus_axes_hifwb_report.md`; pre-outcome rules: `analysis_freeze.md`; source hashes, aggregate runner, figures, verification, and complete artifact hashes are in the same directory. Base is AA-19 commit `f2f16a5224da2277ab8bc946e70b27deaa5108de`. Inputs are AA-19's hash-verified future-safe unique-source direct bridge, fixed five-factor loadings and C1/C2/C3 restricted axis weights, AA-16's frozen 13-item HiFWB definition, the verified external SAPA V5 response matrix and official IPIP100 keys. C1/C2 score weights are frozen projections into the AA-19 five-factor loading span; C3 primary is the frozen sign-aligned H2 pattern and C3 subspace is sensitivity only. Source rows, identifiers, raw data, and absolute paths are not exported. All predictor/outcome overlap is zero before fitting. The AA-13 raw split is reused, with predictor transformations fit only in train or train+validation data. Bootstraps, outcome-label permutations, repeated splits, bridge, factor-count, and dominant-trait sensitivities are CPU-only aggregate outputs. No model inference, activation work, RunPod, paid compute, HiFWB-driven factor fitting, persona projection, or viewer occurs. The provisional bridge, planned SAPA missingness, Big Five overlap, and AA-15 extraction-provenance limitation remain material caveats.

## AA-19 independent SAPA consensus-factor validation

Entry point: `research/outputs/aa19_human_consensus_factor_validation/three_model_human_validation_report.md`; frozen pre-outcome protocol: `analysis_freeze.md`; source hashes, aggregate-only runner, verification, figures, and complete artifact hashes live in the same output directory. Base is AA-18 commit `eefe021f4f76635eb117e121aa824106a9d08c67`. Frozen model inputs are AA-18 signed C1–C5 consensus/model loadings, trait weights, score and stability tables, and the three saved 275 × 240 cosine matrices. Human fitting inputs are the frozen 45/74 AA-16 semantic bridge with exact-source and overlap flags, AA-1's 183 trait-item orientation rows, the official SAPA V5 psychological item-response file (SHA256 `fb480e6bd4c5ba0832cdd105c2fac5dc47b144378e96ffb3a50f3e8d63868cb6`), and official IPIP100 scoring keys. The external raw location is supplied at run time and is not committed. The script verifies source hashes and reproduces the prior 45-proxy correlation matrix, then fits future-safe unique direct human proxy correlations independently of model loadings, Big Five, and all HiFWB outcomes. Model-side bridge reconstruction precedes cross-domain interpretation. Respondent bootstrap/split half, trait-label nulls, bridge, Spearman, eight held-out split-set and five-versus-six-factor sensitivities, and Big Five post-fit comparison are aggregate only. The post-freeze split and factor-count QA is clearly distinguished from the frozen primary selection; C3's individual human match weakens at six factors. No respondent rows or identifiers, HiFWB values, model inference, RunPod, paid compute, or viewer enter outputs. The semantic bridge still awaits independent expert adjudication; model-vector extraction provenance remains incomplete.

## AA-18 three-model consensus trait structure

Entry point: `research/outputs/aa18_three_model_consensus_trait_structure/three_model_consensus_trait_report.md`; frozen protocol: `research/outputs/aa18_three_model_consensus_trait_structure/analysis_freeze.md`; reproducible CPU runner, source hashes, complete artifact hashes, and verification sit in the same directory. Base is AA-17 commit `05ded7f8edd05550e3f9a7e8b18c3926cc51b3c5`. The only fitting inputs are three saved 275 × 240 same-model signed role–trait cosine CSVs (Qwen `trait_persona_prediction`, Llama/Gemma `multimodel_trait_profile_pc_predictor`), matched by exact persona and trait order. Within-model training-fold standardization, rank-20 SVD and ridge MAXVAR-GCCA determine the multiview components. Persona/trait permutation nulls, held-out folds, bootstraps, split halves, rank/regularization sensitivity, leave-one-model-out reconstruction, and AJIVE-like row-subspace partition validate them. AA-17 factors and the frozen AA-16 45/74 semantic bridge enter only after component decisions for comparison/readiness. No human outcomes, respondent-level data, raw tensors, activation PCs, model inference, RunPod, paid compute, or viewer were used. Historical trait extraction records remain incomplete; common labels do not establish measurement invariance, independent psychology, or causal traits.
...
Prospective freeze: `research/outputs/aa15_hifwb_trait_pc/analysis_freeze.md`, generated from the byte-identical AA-10 HiFWB item freeze and the repo extraction-question bank by `build_prompt_freeze.py`. It fixes 13 item identities, six reverse keys, three formulations, five contrastive prompt pairs, eight questions, response-token/all-layer aggregation, seed 150015, and no PCA refit. `extract_hifwb_vectors.py` is an unexecuted GPU runner; no indicator activation vector or AA-15 projection has yet been produced. `pre_extraction_verification.json` checks the freeze and the AA-14 direction files.
...
## AA-26 implementation lineage — 2026-09-20

Source base d4bd3ff; corrected immutable measurement commit51e5f52; affected human refits cf6ce5b; new nested analysis in `run_information_gain.py` / `masked_ridge.py`. Sources are pinned git blobs and the existing hash-matched human raw/key files; no new acquisition. Numerical checks independently compare observed-label ridge to sklearn, test training-fit invariance to held-out values, reverse means and masks, target/predictor separation, fold/group/curve outputs and preserved AA16–25 artifacts. Main verification has 241 checks. Raw rows remain local in memory. `artifact_inventory.csv` hashes active and archived files, excluding itself. The source freeze's working PS:S label is preserved; reporting-only correction to behavioral/emotional self-regulation changes no measures or metrics. The score fingerprint is missing-zero-filled little-endian float64 with a separate observation mask. `build_report.py` renders a static scientific curve, no viewer. Agent analysis model: GPT-6 Astra per user request.

## AA-27 detailed execution provenance

The model-selection section alone changed in AGENTS.md at6d564c3. Official first-party OSF files were downloaded to gitignored data_external/aa27_ipip_neo, verified against archive SHA256, and never exported as respondent rows. Scoring uses the documented already-keyed file orientation. The newer Johnson/Twomey workbook Input sheet was compared to official IPIP HTML;300 signs/facet memberships matched after three documented wording normalizations. The original item-key XLS is acquired/hashed, not claimed as the extraction engine. Dataset OSF node license is null; public-domain item permission is not a dataset license assertion. The semantic7200-row crosswalk and gate were committed in79417c2 before acquired human response density auditing or model schema checks. Incidental published example-norm exposure is disclosed in the freeze and was not used. Numeric model cells were not evaluated; only hashes, persona names and trait headers were verified. Gate failure intentionally leaves downstream CSVs empty with explicit NOT_RUN execution status; the two figures are density and coverage, not fabricated calibration or profile comparisons. Reproduction and verification scripts plus complete artifact hashes accompany the feasibility report. Agent model GPT-6 Astra, High.
