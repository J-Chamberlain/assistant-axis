...
Created: 2026-09-11 by Codex (GPT-5.5) on branch `codex/qwen-trait-sparsity`, from canonical base `6654e2e5f8914790df65038417f46a7d61202c8a`. Statistical estimators were Ridge and MultiTaskElasticNet. No GPU, RunPod, language-model inference, activation extraction, response generation, or external model API was used.
Source inputs: Existing Qwen 275-persona x 240-trait activation-cosine matrix at `research/outputs/trait_persona_prediction/persona_trait_similarity_matrix.csv`; canonical Qwen PC coordinates at `research/visualizations/geometry_viz_data.json`; existing released/local Qwen role and trait tensors under `downloads/hf_vectors/qwen-3-32b/{role_vectors,trait_vectors}`; canonical predictor design and metrics under `research/outputs/trait_profile_pc_predictor/`; exact fixed-15 definitions under `research/outputs/persona_trait_ridge_plots/`; PCA builder `research/visualizations/scripts/build_geometry_viz.py`.
Generating script: `research/outputs/qwen_trait_sparsity_prediction/run_qwen_trait_sparsity.py`.
Procedure: Verified that persona PCA mean-pools 275 stored role tensors to a 275 x 5,120 matrix and fits PCA on activation coordinates without trait inputs. Reproduced the canonical 240-trait Ridge LOPO baseline; ran exact fixed-15 and all 240 single-trait benchmarks; computed outer-test conditional permutation importance; performed joint and PC-specific forward selection entirely inside repeated 5-fold x 10 outer training partitions with 4-fold inner trait/alpha/scaling selection; fit repeated nested MultiTaskElasticNet stability models; grouped correlated trait profiles at absolute r>=0.90/0.95; and generated a descriptive full-data path only after held-out evaluation. Controls used deterministic random real-trait subsets, target-independent isotropic N(0,I_5120) directions, and random directions reconstructed separately inside each outer fold from centered outer-training role vectors only. SVD measured direct PC-loading coverage by real-trait spans; role-vector norm variation was audited separately.
Key result: Full LOPO R2=0.999522/0.998811/0.999611, normalized RMSE=0.045378. Joint selected 5/10/15 reached normalized RMSE=0.176511/0.106880/0.087216; the fixed editorial 15 reached 0.165056. All-PC R2 thresholds .90/.95/.98/.99 were first reached at k=3/3/4/6, and normalized-RMSE thresholds .25/.15/.10/2x-full at k=4/6/12/14. At k=15, random-real/isotropic/persona-span median normalized RMSE was 0.153307/0.737911/0.170184; optimized 15 beat all 100 banks in every matched distribution. At k=240, fold-local persona-span random directions reached 0.051422 versus repeated full-real 0.047609, while isotropic directions reached 0.124511. The full real-trait span covers squared fractions 0.987955/0.985303/0.981947 of PC1/PC2/PC3 loadings. The compact permutation null was near/below chance and every automated verification passed, including a byte-identical second primary fixed-seed run.
Dependent analyses: Claim 21; Paper 1.5 appendix/methods discussion of same-space evidence; future preregistered behavioral validation of a frozen compact/profile prediction against a genuinely new elicited Qwen persona.
Current status: Active and complete. The scientific interpretation is mixed: compact selected real traits outperform matched generic bases at low k, but large generic persona-span banks nearly reconstruct the complete result.
Notes/caveats: Predictors and targets transform the same underlying role vectors; this is not independent psychological validation. The fixed editorial 15 is slightly worse than a median random real-trait 15. Correlated substitutes make exact trait identity unstable, conditional permutation conservative under redundancy, and sparse nonzero frequency non-unique. Target-informed small-span coverage is descriptive rather than held-out evidence. PC-specific compact rankings differ. No causal trait, human psychometric, cross-model, or behavioral-realization claim follows.
