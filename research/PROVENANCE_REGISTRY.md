...
## qwen_pc_trait_specificity

Artifact: Qwen PC1-PC6 cross-PC specificity, associated-versus-defining trait sets, all-trait Pareto frontiers, secondary descriptive composites, and frozen-SAPA coverage refinement.
Location: `research/outputs/qwen_pc_trait_specificity/`; readable report: `qwen_pc_specificity_report.md`; exhaustive trait-target table: `qwen_trait_cross_pc_specificity_all.csv`; all hashes and branch/future-canonical URLs: `artifact_inventory.csv`.
Created: 2026-09-12 by Codex on `codex/aa1-qwen-pc-trait-specificity`, starting exactly at validated AA-1 commit `667acdd634bfb8c39d40432fe60f0c5ef3bbbeeb`. The primary rule was committed at `595c1e22cc5e7cd7b7e9a6a90046e18b9b103d68` before trait-specific results were computed or inspected.
Qwen sources: the saved 275 x 240 Qwen activation-cosine trait matrix; the exact Qwen-only PC1-PC6 role-coordinate object; the 1,440-row AA-1 correlation table; the unchanged 328-row AA-1 membership table; canonical trait definitions; and the indexed Qwen Affiliation visualization records. Shared visualization files were stream-filtered to Qwen rows so no Llama/Gemma numerical value entered analysis.
Procedure: for every trait-target pair, retain all six signed correlations and compute target strength, largest off-target, margin, ratio, and PC1-PC6 concentration. A single fixed-seed set of 2,000 role resamples is paired across all six PCs. A PC-defining trait must remain an unchanged AA-1 member and have target-is-largest probability at least 0.95. Observed concentration at 0.75 and 0.50 defines descriptive high/moderate tiers, not tests. The Pareto frontier spans all traits within each PC/pole for target strength, margin, and concentration. Full 0.40/0.50/0.60 by 0.90/0.95/0.99 sensitivity is retained.
Observed outputs: the 328 associated memberships contract to 202 PC-defining and 70 highly concentrated memberships. Signed associated/defining counts are PC1 47/35 positive and 123/95 negative; PC2 31/12 and 44/23; PC3 42/18 and 31/18; PC4 2/0 and 6/1; PC5 1/0 and 0/0; PC6 0/0 and 1/0. The canonical Affiliation percentile composite is exactly `empathetic`, `agreeable`, and `altruistic` at equal weights and has PC3 r=-0.944857, largest off-target PC1 r=-0.110061, margin 0.834796, and PC1-PC6 concentration 0.978101.
Human source and boundary: the 48 frozen AA-1 SAPA judgments are unchanged. Added fields only intersect each judgment's covered traits with defining, highly concentrated, and Pareto sets. Explicit mapped coverage changes descriptively from 145/328 associated memberships to 93/202 defining memberships; this is qualitatively more focused and uneven, not a correspondence estimate.
Verification: `verification_report.json` passes 29 checks, including independent observed correlations and specificity metrics, a second fixed-seed bootstrap using distinct Pearson algebra, independent Pareto reconstruction, exact AA-1 reproduction, exact PC3 formula recovery, unchanged human judgments, deterministic rerun, parsing, hashes, privacy, and firewalls.
Evidentiary boundary: Qwen model-side characterization and human-measurement availability only. No human respondent was scored/projected; no human/model correspondence test, Llama/Gemma comparison, AA-7 result use, or next-experiment selection occurred. No respondent-level microdata, GPU, RunPod, inference, activation extraction, response generation, or external model API was used.
...
Location: `research/outputs/qwen_trait_family_human_inventory/`; main machine-readable deliverable: `qwen_pc_trait_family_human_inventory.csv`; readable report: `qwen_pc_trait_family_human_inventory_report.md`; all hashes and raw URLs: `artifact_inventory.csv`.
Created: 2026-09-12 by Codex on `codex/aa1-qwen-trait-family-human-inventory`, starting from validated AA-1 source commit `313c5cff6e071d707d37b1318343cde4ed510725`. Numeric selection was frozen at `9ce2809`; the neutral mapping packet at `70f7617`; human judgments at `9c5aad1`; and the assembled inventory at `921df92`, all before canonical-state reconciliation.
Qwen sources: `research/outputs/trait_persona_prediction/persona_trait_similarity_matrix.csv` (275 roles x 240 Qwen activation-cosine traits), the exact Qwen object isolated from `research/outputs/extended_persona_pca/viewer_data.json` (PC1-PC6), canonical PC1-PC3 reference geometry, and the 240 canonical trait definitions. Pearson, Spearman, analytic p-values, PC-local BH-FDR, ranks, and 2,000 role-bootstrap sign estimates are retained for all 1,440 PC-trait pairs.
Selection procedure: primary membership requires `|Pearson r| >= 0.50`, `BH-FDR q < 0.01` within each PC, and bootstrap sign stability at least 95%; 0.40 and 0.60 sensitivities use identical FDR/stability rules. Positive and negative poles remain separate and empty families are not relaxed. The selection specification was committed before semantic description.
Human sources: the validated 126-construct `human_construct_library.csv`, its source documentation, the 696-item SAPA dictionary, and corrected future wording metadata for the eight previously flagged items. The blind reviewer packet excluded PC IDs, role/persona evidence, prior interpretations, AA-2/AA-7 results, prior AA-1 hypotheses, and Llama/Gemma results. The full construct library was the candidate universe; item-only fallback was permitted where no scale represented the selected content.
Observed outputs: 328 memberships spanning 238 traits. Forty-eight mapping rows use 37 unique construct/item candidates and 350 unique SAPA items. Four signed families have a strong available match, five have partial matches, one has facet/item-only coverage, and two empty poles have no counterpart. Reuse is explicit: seven constructs and 144 items recur across families.
Verification: `verification_report.json` passes 34 checks, including independent BH reproduction, exact threshold membership, fixed seeds, canonical-coordinate tolerance, blind-packet exclusions, historical-packet preservation, candidate/item integrity, deterministic full rerun, and privacy/firewall checks.
Evidentiary boundary: inventory and semantic measurement availability only. No respondent record was loaded, scored, imputed, or projected; no human/model correspondence was tested; no Llama/Gemma or AA-7 result informed matching; no next experiment was selected. No GPU, RunPod, model inference, activation extraction, response generation, or external model API was used.

## qwen_pc_human_construct_bridge

Artifact: Independent blinded SAPA semantic-review agreement, flagged-item metadata audit, and Qwen-first broad human-construct bridge.
Location: `research/outputs/qwen_pc_human_construct_bridge/`; main report: `qwen_pc_human_construct_bridge_report.md`; frozen hypotheses: `qwen_derived_human_construct_hypotheses_v1.csv`; complete safe-artifact hashes and branch/future-canonical URLs: `artifact_inventory.csv`.
Created: 2026-09-12 by Codex on branch `codex/aa1-qwen-human-construct-bridge`. The exact user-supplied Claude Opus 5 judgment file is preserved byte-for-byte as `sapa_category3_blinded_review_judgments.csv` (SHA256 `71902fbe78fd4dec9742af8ca25a1c27dd9429263f3319a85937b85fd3a9a86a`). Review agreement was committed before Qwen construct selection; the model-side signature and rubric were frozen at commits `b34864b`/`8d791c2`; hypotheses were frozen at `b35c3ea`; the validation plan and report followed at `08bde9a`.
External-review provenance: reviewer Claude Opus 5 (Anthropic), review date 2026-09-11, with no prior substantive project knowledge reported. The reviewer located only the three requested review files and reported no access to project state, claims, geometry, prior decisions, sparsity, occupations, NLSY97 outcomes, or downstream results. Anthropic-system status is recorded as a structural conflict. This is separate AI content adjudication, not independent human/expert psychometric validation.
Human sources: unchanged Harvard Dataverse SAPA V5 release, DOI `10.7910/DVN/SD7SVE`; official IPIP/SPI documentation for construct definitions and flagged wording checks; existing reviewed SAPA bridge and human-only structural audit. Raw SAPA responses remain gitignored under `data_external/human_validation/sapa/doi_10.7910_DVN_SD7SVE/`. Only 331 `q_*` item columns needed for selected-construct feasibility were read; no respondent identifier was read or emitted.
Qwen-only sources: Qwen/Qwen3-32B PC1-PC6 variance/stability, all 240 trait associations per PC, role extremes, Qwen-filtered Big Five scores, existing Qwen interpretations, AA-4 marginal/conditional evidence, and the PC1 accountability intervention. Canonical PC1-PC3 coordinates reproduce to max absolute error `1.207027046312703e-06`. Llama, Gemma, and AA-7 result files were excluded from construct selection and are absent from the source manifest.
Procedure: compare the 78 frozen Codex and Claude decisions without altering either; audit eight Claude-flagged items against primary sources while leaving historical packets frozen; build a 126-construct library spanning domain/aspect/facet/narrow levels; freeze a multi-evidence match rubric; construct Qwen PC1-PC6 signatures; evaluate 48 candidates; freeze 37 Qwen-derived hypotheses; then assess human measurement availability and write a future held-out-validation plan. The frozen set contains 4 strong, 12 plausible partial, and 21 facet/subcomponent rows; 36/37 are SAPA-available.
Dependencies: the independent-review artifacts constrain semantic confidence and metadata corrections; the construct library, Qwen signature, and rubric determine `qwen_pc_human_construct_candidates.csv`; those candidates determine the frozen hypothesis and measurement-feasibility tables. The future validation plan depends on the already frozen hypotheses and must not be used to revise them post hoc.
Evidentiary boundary: discovery-only bridge construction. No human respondent was projected; no human/model equivalence, cross-model replication, causal mechanism, or behavioral realization was tested. No GPU, RunPod, model inference, activation extraction, response generation, or external model API was used.
