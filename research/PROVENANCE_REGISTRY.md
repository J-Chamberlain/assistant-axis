...
## existing_activation_sample_audit

Created 2026-09-24 by GPT-6 Astra. Active local-only audit at research/outputs/existing_activation_sample_audit/, based on original checkout 0693aaf76e5073c37377024130dbc2d0e80424fd; successor consulted at 621ef9b53adcc957385cfad19655c8c03a304661. source_manifest.json pins all three public releases and hashes local evidence; inventory.csv explicitly covers every model/persona/trait aggregate and missing original sample set. Static pickle metadata inspection, local weights_only CPU spot loads, numpy allow_pickle=False, existing score counts and coordinate joins only. No downloaded code execution, new judging, inference, extraction, GPU, migration or push. Third-party headers are not full payload verification. Original samples not found in inspected sources are distinguished from inaccessible private/remote archives. Initial ENOSPC preservation failure and one specifically presented SciPy wheel-cache cleanup are documented in disk_preservation.json. No research artifacts or Git history deleted.
