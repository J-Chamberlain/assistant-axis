...
Location: `research/outputs/nlsy97_occupation_personality_stability/`; main report: `nlsy97_occupation_personality_stability_report.md`; frozen specification: `analysis_specification.md`; complete hashes and branch/future-canonical URLs: `artifact_inventory.csv`.
Created: 2026-09-11 by Codex. The outcome-blind scoring, occupation hierarchy, resampling, sample-size curve, discriminability, and feasibility-tier rules were committed first at `13fb08f1d75444820d0b374fef2e3e0292a4d1f1` from canonical starting SHA `7914d01f7d9c23060fe199815a90e547241d9682`.
Human source: Official BLS/CHRR NLSY97 public-use extract obtained through the normal NLS Investigator guest workflow. The exact reconstructed 126-variable tagset, codebook, and 8,984-row CSV match canonical AA-1 SHA-256 hashes `3efd27d...`, `08bc874...`, and `b55b543...`; all remain gitignored under `data_external/human_validation/nlsy97/`.
Repository inputs: The nine canonical AA-1 NLSY97 metadata/count/source artifacts recorded in `source_manifest.json`, including the official 2002 Census occupation list and the existing conservative persona-to-occupation crosswalk. Four erroneous Round-6 direction labels in the AA-1 convenience item manifest were corrected from the official extraction codebook; the analysis had already frozen and used the official directions.
Procedure: Score official two-item TIPI domains; globally standardize them with the Round-12 cumulative-case weight; form weighted centroids at 2002 Census occupation, 2000 SOC minor-group, and 2000 SOC major-group levels fixed before outcome inspection; estimate Taylor VSTRAT/VPSU intervals, 500 design-respecting PSU bootstraps, 500 deterministic within-cell split halves, and 300 repeated subsamples at each supported N; add empirical-Bayes shrinkage, Round-6 A/C recurrence, variance decomposition, and fixed cross-validated discriminability sensitivities. Suppress cell-specific outputs below N=10. Apply frozen tiers only after all human stability estimates exist, then join the pre-existing role crosswalk to list future candidates.
Key result: Of 151 reportable narrow cells, 9 are strong and 18 moderate/exploratory; of 22 reportable broad families, 18 are strong and 2 moderate. Expected five-dimensional centroid error is about 0.43-0.46 at N=20, 0.26-0.28 at N=50, and 0.17-0.18 at N=100. Sampling-error-corrected survey-weighted ICCs are 0.038-0.078 at the narrow level and 0.008-0.032 at the broad level, so precision of broad centroids coexists with weak occupation differentiation and dominant within-occupation heterogeneity. Classification is only 0.009-0.034 balanced-accuracy points above the multiclass baseline. Round-6/Round-12 recurrence is mixed and more consistent for Conscientiousness than Agreeableness.
Role-candidate result: Secretary/administrative assistant is strong at the narrow level; accountant/auditor, caregiver, and the `production workers, all other` artisan proxy are moderate. Counselor and several sparse professional/creative cells have only aggregated stable parents with explicit semantic cost; lawyer has no stable reportable parent. The 94 output rows are future mapping candidates only.
Verification: `verification_report.json` passes 19/19 scoring, design, aggregation, suppression, privacy, boundary, and reproducibility checks; a complete rerun exactly matches all 15 deterministic generated-artifact hashes.
Current status: Active and complete human-data feasibility evidence for later preregistration. It does not establish model/human correspondence or construct identity.
Notes/caveats: TIPI is a very short measure; within-occupation variance dominates; N alone does not determine a tier; stable official parents can impose unacceptable semantic cost. No model geometry, activation vectors, model traits, persona coordinates, model inference, human-to-model projection, GPU, RunPod, activation extraction, or external model API was used. No respondent IDs or row-level NLSY97 data are committed.
