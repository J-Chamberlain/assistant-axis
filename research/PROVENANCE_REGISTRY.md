...
UI revision (2026-09-13, comparison): Moved profile charts beneath the scatter; a selectable saved second persona supplies a second percentile line, a first-minus-second zero-centered difference chart, and an actual-to-actual PC connector in 2D/3D. The saved data and held-out model bundles remain unchanged; the update is descriptive visualization only, with browser checks for exact values and coordinates.
...
Source inputs: `research/outputs/trait_profile_pc_predictor/{trait_profile_pc_predictor_report.md,ridge_predictor.json,leave_one_persona_out_predictions.csv,ood_reference.json,predict_trait_profile.py,run_trait_profile_pc_predictor.py,source_manifest.json}`; `research/outputs/trait_persona_prediction/persona_trait_similarity_matrix.csv`; `research/visualizations/{geometry_viz_data.json,persona_geometry_explorer.html}`; `research/visualizations/scripts/build_geometry_viz.py`; `data/traits/trait_list.json`; and UI concepts inspected in `research/outputs/{persona_trait_ridge_plots,persona_trait_surface_viewer}/`.
Generating script: `research/outputs/trait_profile_geometry_explorer/run_trait_profile_geometry_explorer.py`; reproducible interface sources: `viewer_template.html`, `trait_profile_geometry_core.js`, and `viewer.js`.
Procedure: Verified exact 275-persona and 240-trait identity/order/finite integrity; repeated the canonical raw-cosine LOPO fitting procedure 275 times, each with one persona removed and alpha tuned by shuffled four-fold inner CV on the other 274 using seed `10000+i`; exported direct raw-input-to-PC coefficients/intercepts; embedded the canonical geometry, profiles, final all-persona model, q95 errors, and OOD reference; and mirrored the CLI's empirical percentile/inverse-quantile and OOD calculations in browser-readable JavaScript. Selected-persona edits continue through the same model that excludes that persona.
Outputs: 275-model transparent `lopo_ridge_predictors.json`; `trait_profile_geometry_data.json`; 30-case `browser_python_reference_cases.json`; source-built standalone companion HTML; Python, Node, and live-Chrome verification reports; methodology/report; source manifest; and active artifact inventory.
Verification: Generator maximum LOPO reproduction error 2.32e-12; independent Python 2.08e-12; JavaScript core and live Chrome 2.27e-12 across all 275 baselines. Thirty deterministic modified cases agree with Python within 5.68e-13 for PC predictions in core tests and 1.42e-14 for OOD scalars; live Chrome also verified list/plot selection, native sliders, changed-only filtering, reset, projection, and camera persistence. Existing trait visualization regression tests pass. Canonical explorer hash remained `82d8e7f3d659ca441551c5db06ec5bba508f669fb3bf54abe184f38ef3277dce`.
Dependent analyses: Manual exploration and selection of counterfactual profiles before the separately preregistered novel-persona behavioral-validation experiment. The viewer does not itself validate a new persona.
Current status: Active visualization companion. It does not replace the canonical persona geometry explorer or canonical V1 predictor.
Notes/caveats: Features and targets share Qwen role-vector provenance. Percentiles are corpus-relative controls; edited coordinates are predicted counterfactual locations, not observed behavior or causal effects. The deployed OOD reference is an all-corpus geometric context with the selected persona excluded from neighbor lookup, not a persona-held-out uncertainty model or calibrated probability. No claim or finding status changed.
