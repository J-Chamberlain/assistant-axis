...
Current grouped-trait landscape (2026-09-11): `research/outputs/persona_trait_surface_viewer/persona_trait_surface_viewer.html` now switches without reload among Qwen, Llama, and Gemma. Every model uses its own coordinates, within-model trait ranks, equal-weight three-trait group heights, fitted fabrics, support masks, flat planes, and diagnostics. Qwen remains the default and reproduces the prior saved bundle exactly. Camera/view settings and a selected persona persist by role name across model switches; rapid-switch stale-trace checks and actual local Chrome 152/Plotly WebGL inspection passed. The groups remain editorial reading groups, not fitted factors, and cross-model percentile equality is rank equality rather than absolute psychometric equivalence. No inference, activations, GPU, RunPod, external model API, or claim change.

Current trait companion (2026-09-11): `research/outputs/persona_trait_ridge_plots/persona_trait_ridges.html` now switches without reload among Qwen, Llama, and Gemma while retaining the same 15 traits and five editorial groups. The 12,375-row bundle contains each model's own raw activation cosines, population z-scores, within-model percentiles, and PC1/PC2/PC3 ordering. Qwen uses canonical geometry; Llama/Gemma use their own role-vector PCA with the established multimodel sign orientation. Qwen's prior 4,125 rows and coordinates reproduce exactly. Static exports, DOM tests, and actual local Chrome 152 inspection passed. These are same-space profiles and within-model ranks, not absolute cross-model psychological measurements. Claims unchanged.
