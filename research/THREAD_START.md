...
Startup follow-up (2026-09-09): the user reported that the local emotion viewer stayed on its initial loading message. A UI-only patch now shows the prepared static preview, reports early JavaScript/renderer errors and stages, and times out after 15 seconds. The original browser tests are historical, not confirmation in the user's profile. Node/DOM-double diagnostics tests pass; actual user-profile startup and the root cause remain pending the user's refresh/error report. Browser-tool inspection of this local-file tab was blocked, and that restriction was not bypassed. No score or mesh data changed.
