...
DEFAULT_PHASE2_PATH = OUTPUT_DIR / "trickster_phase2_scores_gpt41mini.jsonl"
INTEGRITY_PATHS = [
...
def main() -> None:
    ...
    parser.add_argument("--scores-path", type=Path, default=DEFAULT_PHASE2_PATH)
    parser.add_argument("--output-prefix", default="trickster_vector_validation")
    parser.add_argument("--allow-partial", action="store_true")
    args = parser.parse_args()

    phase2_path = args.scores_path
    if not phase2_path.is_absolute():
        phase2_path = REPO_ROOT / phase2_path
    ...
    if len(scores) < EXPECTED_RECORDS and not args.allow_partial:
        raise SystemExit(
            f"Phase 2 scoring not complete; found {len(scores)} scored records, "
            f"expected {EXPECTED_RECORDS}. Use --allow-partial for adaptive Codex-scored validation."
        )
    ...
    validation = {
        ...
        "partial_scoring_allowed": bool(args.allow_partial),
    ...
    (OUTPUT_DIR / f"{args.output_prefix}.json").write_text(json.dumps(validation, indent=2) + "\n")
...
- Partial scoring allowed: `{bool(args.allow_partial)}`
...
"""
    ...
    (OUTPUT_DIR / f"{args.output_prefix}.md").write_text(report)
