...
## Dated Update: Qwen Trait Association Versus PC Specificity (2026-09-12)

Observed: preserving the frozen AA-1 salience rule while requiring the target to be the largest absolute PC1-PC6 association in at least 95% of 2,000 paired role bootstraps reduces 328 signed memberships to 202 PC-defining memberships; 70 are highly concentrated at observed PC1-PC6 concentration at least 0.75. PC1 positive/negative contract from 47/123 to 35/95, PC2 from 31/44 to 12/23, PC3 from 42/31 to 18/18, PC4 from 2/6 to 0/1, and PC5-PC6 retain no defining member.

Observed for PC1: `transparent`, `factual`, `analytical`, and `methodical` remain highly concentrated PC1-positive traits. `conscientious` is broad/cross-PC: PC1 r=0.693 versus PC2 r=-0.699, giving margin -0.005 and concentration 0.487. This supports keeping disciplined/factual/analytical language while not treating every conscientiousness-related label as specific to PC1.

Observed for PC3: positive `callous`, `cynical`, and `pessimistic`, and negative `benevolent`, `nurturing`, `supportive`, and `altruistic`, are highly concentrated. `agreeable`, `forgiving`, `collaborative`, and `tactful` are moderately concentrated; `empathetic`, `accommodating`, and `conciliatory` remain strong PC3 associations but are broad/cross-PC. The exact Qwen Affiliation visualization composite—equal-weight mean of within-Qwen percentiles for `empathetic`, `agreeable`, and `altruistic`—is strongly specific as a group (PC3 r=-0.945; largest off-target PC1 r=-0.110; concentration 0.978), even though one constituent is individually broad.

Interpretation: specificity strengthens the diagnostic use of the PC3 cooperative-care versus antagonistic family while showing that individual social traits differ in exclusivity. It also reinforces the low semantic yield of PC4-PC6 under a strict single-trait dominance criterion. This is Qwen same-space characterization, not a human/model correspondence result or cross-model generalization.
