...
## Existing activation samples: bounded availability claim (2026-09-24)

Status: Observed inventory result, not a global nonexistence claim.
Claim: Original-study response distributions were not found in the audited sources for any of the 275 personas or 240 traits in each model. Local later Qwen evidence covers four personas, not the full vocabulary; third-party selected-role activations remain a separate provenance/compatibility lead.
Limits: Layer rows are not samples; centroids do not reconstruct distributions; projection-only clouds omit discarded dimensions. Fixed-vector response-score uncertainty does not estimate trait-vector uncertainty. No gregariousness polarity reconstruction or human structural comparison was performed. AA15/AA26 corrections and AA27's failed primary bridge gate remain unchanged.
Evidence: research/outputs/existing_activation_sample_audit/report.md, methods.md, inventory.csv, source_manifest.json.
