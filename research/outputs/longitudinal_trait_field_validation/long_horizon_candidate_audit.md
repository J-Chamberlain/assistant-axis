...
The long-horizon program is feasible in more than one cohort, but the cohorts are not interchangeable. LISS currently offers the strongest combination of repeated measurement frequency and lower-level item richness. HRS offers a strong second design with 26 stable adjective items over repeated four-year intervals in two rotating subsamples. HILDA offers a 16-year nationally representative panel with a relatively rich adjective battery, but published work finds metric rather than scalar invariance for the broad Big Five, so exact project-specific partial-scalar checks are required before confirmatory use. MIDUS offers an unusually long approximately 19-year span and a rich 25-adjective MIDI battery, but its current data-use terms explicitly prohibit uploading MIDUS data to public large-language-model or AI platforms. SOEP offers a large 12-year core panel and clean repeated item wording, but its 15-item BFI-S is much thinner and published scalar invariance fails, making it a useful multi-domain replication rather than the strongest primary cohort.
...
Measurement status: published LISS analyses support metric invariance across repeated personality waves. Scalar invariance is described as ambiguous rather than clearly established. Therefore the current audit does not label LISS M1. Project-specific longitudinal partial-scalar tests on the exact bridge item sets are a required pre-outcome measurement gate. If those fail, primary interpretation must remain directional and measurement-qualified under the frozen protocol.

Known wave issues: prior summaries note no personality administration in 2016 and substantial missingness from survey errors in some earlier waves. These are design/coverage issues, not reasons to select favorable intervals. Every compatible interval must be enumerated before outcomes are inspected.
...
Provisional status: `HIGH-COVERAGE; measurement gate pending; local execution required by current access terms`.
...
Measurement status: exact wording is documented across waves, but this audit did not identify a published longitudinal scalar-invariance result covering the exact common-26 bridge. A project-specific invariance audit is therefore required before assigning M1. Until then the wording continuity supports an M2 candidate classification.
...
Access boundary: HRS public-release files require individual registration. The official Conditions of Use prohibit transfer of HRS data to third parties except under specified institutional arrangements. Raw HRS data should not be uploaded into this ChatGPT thread. Use an authorized local execution path and return only permitted aggregate results.

Provisional status: `HIGH-COVERAGE; M2 candidate pending invariance audit; local execution required by current access terms`.

## 3. HILDA: strong long-horizon population cohort with measurement caveat

Observed source structure: HILDA administers a 36-adjective Big Five battery derived largely from Saucier/Goldberg markers. The standard HILDA broad scales use 28 recommended items. Recent coordinated work identifies personality assessments in 2005, 2009, 2013, 2017, and 2021, giving a 16-year span. Earlier official/report sources document identical item wording across the first four waves.

The bridge can support strong or plausible lower-level targets for Sociability and Energy Level; Compassion and Respectfulness; Organization, Productiveness and Responsibility; Anxiety and Emotional Volatility; and Intellectual Curiosity and Creative Imagination. Depression, Trust, Assertiveness, and Aesthetic Sensitivity do not have clean mappings. Two potentially useful target indicators, Harsh for Respectfulness and Careless for Responsibility, are among the eight adjectives excluded from the standard broad HILDA scales because of cross-loading/psychometric concerns. They may be used only if the preregistered baseline psychometric gate supports them; otherwise they are downgraded to PROXY. HILDA therefore has 9 robust candidate facets and up to 11 if those two pass. It is guaranteed MULTI-DOMAIN ELIGIBLE and may become HIGH-COVERAGE after the measurement audit.

Leakage is manageable because the broad scales contain four to six items per domain and the eight excluded adjectives create additional target-only evidence. Every target must use a target-specific leave-out broad score where overlap remains.

Measurement status is the central issue. Published longitudinal modeling over 2005, 2009, 2013, and 2017 supports metric invariance but explicitly reports that scalar invariance could not be established, particularly for Openness and Agreeableness. Under the frozen protocol, HILDA therefore cannot be assumed M1. Before outcome analysis, the exact lower-level bridge and leave-out broad item sets require project-specific partial-scalar tests. Failure of that gate will downgrade affected intervals/facets to exploratory use rather than trigger remapping.

Access boundary: HILDA unit-record data require an approved DSS/ADA application and Confidentiality Deed Poll. DSS guidelines state that Unit Record Data must not be shared with anyone who is not an authorized user of the same dataset. Raw HILDA data therefore should not be uploaded into this ChatGPT thread unless the platform is explicitly approved under the user's authorization. The default plan is local execution.

Provisional status: `MULTI-DOMAIN guaranteed; possible HIGH-COVERAGE; published scalar-invariance failure requires project-specific measurement gate; local execution`.
...
Measurement status: published work supports strong factorial structure and age-group invariance of the MIDUS adjective items, but this audit did not locate a longitudinal scalar-invariance result for the exact three-wave bridge. The cohort remains M2-candidate pending a project-specific wave-invariance audit.

Access boundary: MIDUS explicitly states that users must not upload MIDUS data to public large-language-model or AI platforms including ChatGPT, Claude, Gemini, and Copilot. Therefore participant-level MIDUS analysis cannot be performed in this chat. A local non-AI analysis runner is required. This restriction is stronger and clearer than the general no-redistribution restrictions on the other cohorts.
...
Measurement status: published longitudinal work finds metric but not scalar invariance for the SOEP Big Five across repeated waves. As with HILDA, project-specific partial-scalar tests on the exact single-item targets and leave-target-out broad scores are required before confirmatory use. The 2005 absence of the fourth Openness item is handled by retaining the common three-item broad Openness definition across the long 2005-2017 interval; Intellectual Curiosity is simply unavailable for that interval rather than changing the broad coordinate definition.
...
Provisional status: `MULTI-DOMAIN ELIGIBLE; lower-resolution replication; scalar-invariance gate required; local execution by default`.
...
- HILDA: 9 robust facets, up to 11 after two excluded-item psychometric gates, across 5 domains; MULTI-DOMAIN guaranteed, possible HIGH-COVERAGE.
...
Unknown: exact eligible person-interval N, project-specific partial-scalar invariance, baseline coherence of marginal HILDA mappings, attrition effects, and final DIRECT/CLOSE counts cannot be known until authorized microdata are processed locally. None of these gates may be relaxed based on longitudinal prediction performance.

## Principal external sources consulted

HILDA: Melbourne Institute HILDA user manual/questionnaire documentation; Losoncz HILDA personality-scale development; recent coordinated HILDA/HRS/LISS personality-wave summaries; Seifert et al. longitudinal measurement-invariance analyses; DSS data-access guidelines.

HRS: official HRS Psychosocial and Lifestyle Questionnaire 2006-2022 user guide and cross-wave concordance; HRS public-data Conditions of Use.
...
MIDUS: MIDUS official data-access policy and personality documentation; published MIDI psychometric work and three-wave longitudinal descriptions.
