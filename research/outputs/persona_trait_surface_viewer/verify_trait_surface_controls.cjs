...
function assertSelectedModelTraces(modelKey){
  const api=window.__traitViewer,model=data.models[modelKey],snapshot=api.snapshot();
...
async function check(){
  await ready();const api=window.__traitViewer;assert.equal(api.state.model,'qwen');assert.equal(elements['model-select'].value,'qwen');
  ...
  elements['reset-view'].emit('click');await ready();assert.equal(api.state.model,'qwen');assert.equal(api.selectedPersona,'playwright');close(camera.fromCamera(api.state.camera).zoom,100);assertSelectedModelTraces('qwen');
  ...
  const result={status:'pass',camera_roundtrip_cases:mathCases,poles_checked:true,models_tested:models,
    checks:['Compiled HTML scripts parse and embedded multimodel data matches','Default and reset model are Qwen','All five groups and fixed color limits',
      ...
      'Repeated and queued Qwen/Llama/Gemma switches converge without stale traces','Selection persists by persona name across models','Yaw/pitch/roll/zoom survive model, axis, group and smoothing changes',
