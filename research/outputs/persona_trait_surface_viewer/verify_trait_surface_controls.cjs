...
const data=JSON.parse(fs.readFileSync(path.join(here,'persona_trait_surface_data.json')));
...
assert.deepEqual(JSON.parse(scripts[2]),data,'Embedded data matches saved bundle');
scripts.forEach((source,i)=>{if(i!==2)new vm.Script(source);});
...
const window={Plotly,TraitCamera:camera,innerWidth:1400,matchMedia:()=>({matches:false}),
  viewerBoot:{stage(){},ready(){},fail(e){error=e;}}};
const context={window,document,Plotly,TraitCamera:camera,structuredClone,performance,console,setTimeout,
  requestAnimationFrame:fn=>setTimeout(fn,0)};
vm.runInNewContext(fs.readFileSync(path.join(here,'viewer.js'),'utf8'),context);
async function ready(){for(let i=0;i<1000;i++){if(error) throw error;if(window.__traitViewer.ready)return;await tick();}throw Error('Render did not settle');}
async function set(id,value,event='change'){elements[id].value=String(value);elements[id].emit(event);await ready();}
async function check(){
  await ready();const api=window.__traitViewer;
  assert.equal(plot.data[4].x.length,275);assert.equal(elements['category-stops'].children.length,5);
  for(let g=0;g<5;g++){
    await set('category-slider',g,'input');assert.equal(api.state.category,g);
  ...
  await set('category-slider',2,'input');await set('smoothing',2);assert.equal(JSON.stringify(api.state.camera),kept);
  ...
  // Queue camera updates and category changes together; the last intent must win.
  for(let yaw=-70;yaw<=70;yaw+=10){elements['camera-yaw'].value=String(yaw);elements['camera-yaw'].emit('input');}
  elements['category-slider'].value='4';elements['category-slider'].emit('input');await ready();
  close(camera.fromCamera(api.state.camera).yaw,70);close(camera.fromCamera(liveCamera).yaw,70);
  assert.equal(api.state.category,4);
  liveCamera=camera.toCamera({yaw:125,pitch:10,roll:-14,zoom:80});
  plot.emit('plotly_relayout',{'scene.camera':liveCamera});
  ...
  const result={status:'pass',camera_roundtrip_cases:mathCases,poles_checked:true,
    ...
    note:'Plotly/DOM double tests UI state and camera math, not actual WebGL rendering. Local browser URL was blocked by browser-use policy.'};
