...
for key,v in d['views'].items():
    ...
    assert len(v['flat_grids'])==5 and len(v['flat_plane'])==5
    assert v['x']==old['views'][key]['x'] and v['y']==old['views'][key]['y']
    for flat_grid,grid in zip(v['flat_grids'],v['levels'][0]['grids']):
        assert len(flat_grid)==61 and all(len(row)==61 for row in flat_grid)
        assert np.array_equal(np.asarray([[x is None for x in row] for row in flat_grid]),
                              np.asarray([[x is None for x in row] for row in grid]))
        array=np.asarray([[np.nan if x is None else x for x in row] for row in flat_grid])
        assert np.nanmin(array)>=0 and np.nanmax(array)<=100
        assert np.isfinite(array[~np.isnan(array)]).all()
    for plane in v['flat_plane']:
        assert len(plane['coefficients'])==3
        assert np.isfinite(plane['coefficients']).all()
        assert plane['node_rmse']>=0
        assert plane['node_r2'] is None or np.isfinite(plane['node_r2'])
    for level in v['levels']:
        assert len(level['grids'])==5
        assert len(level['flat_adherence'])==5
...
result=dict(status='pass',personas=275,groups=5,rows=1375,surface_variants=45,
    ...
    flat_plane_schema=True,flat_adherence_schema=True,self_contained_html=True,no_pca_refit=True,new_activations=False)
