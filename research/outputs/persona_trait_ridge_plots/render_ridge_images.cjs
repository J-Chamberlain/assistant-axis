...
async function main() {
  ...
  for (const suffix of ['pc1', 'pc2', 'pc3', 'overview']) {
    const base = path.join(here, `persona_trait_ridges_${suffix}`);
  ...
  const html = fs.readFileSync(path.join(here, 'persona_trait_ridges.html'), 'utf8');
  const script = [...html.matchAll(/<script>([\s\S]*?)<\/script>/g)][0][1];
  const data = JSON.parse(fs.readFileSync(path.join(here, 'persona_trait_ridge_data.json')));
