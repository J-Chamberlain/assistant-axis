...
def model_panels(model_key: str, model: dict[str, object], curve_values: np.ndarray) -> str:
    ...
    return (
        f'<div class="model-view" data-model-view="{model_key}"{hidden}>'
        f'{"".join(panels)}</div>'
    )


def make_html(models: dict[str, dict[str, object]], model_curves: dict[str, np.ndarray]) -> str:
    ...
    views = "".join(model_panels(key, models[key], model_curves[key]) for key in MODEL_ORDER)
...
footer{{color:var(--muted);font-size:10px;padding:7px 12px}}.notes{{padding:0 26px 24px;max-width:1200px}}.notes p{{font-size:12px;margin:10px 0}}.notes summary{{cursor:pointer}}#selection-status{{color:var(--accent);font-size:11px}}#model-provenance{{font-size:11px;color:var(--accent)}}
...
</style></head><body><div class="intro"><div class="eyebrow">Persona geometry / <span id="active-model-name">Qwen</span> / 15 of 240 mapped traits</div>
...
<p>Left to right: exploration, response, scrutiny, challenge, affiliation. These are editorial reading groups, not fitted latent factors.</p><p id="model-provenance">Qwen uses canonical geometry_viz_data coordinates and exact saved Qwen trait cosines.</p></div>
...
<p>All scores are same-space activation-cosine profiles, not independent psychological ratings. Model selection does not establish identical psychological semantics across models.</p>
...
(() => {{
 ...
 const modelSelect=document.getElementById('model-select'),picker=document.getElementById('find-persona');
 const views=Array.from(document.querySelectorAll('[data-model-view]'));
 const rows=Array.from(document.querySelectorAll('.ridge-row'));
 let model='qwen',selected='';
 function activeRows(){{return rows.filter(row=>row.dataset.model===model);}}
 function select(name,scroll=true){{
   selected=name;
   const matches=[];
   rows.forEach(row=>{{const active=Boolean(name)&&row.dataset.model===model&&row.dataset.persona===name;
 ...
 function setModel(next){{
   ...
   views.forEach(view=>view.hidden=view.dataset.modelView!==model);
 ...
 document.getElementById('reset-view').addEventListener('click',()=>{{selected='';setModel('qwen');select('');}});
 window.__traitRidgeViewer={{get model(){{return model;}},get selectedPersona(){{return selected;}},setModel,select,reset(){{selected='';setModel('qwen');select('');}},activeRows}};
 setModel('qwen');
}})();
</script></body></html>'''
...
def main(vector_root: str | None, qwen_reference_commit: str) -> None:
    ...
    (HERE / "persona_trait_ridges.html").write_text(make_html(models, model_curves))
