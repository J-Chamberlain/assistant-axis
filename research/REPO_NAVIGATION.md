...
Updated UTC: 2026-09-11T23:33:10Z
Last commit when updated: 48e6f23 (externally anchored Big Five audit and viewer integration)
...
Externally anchored Big Five audit: `research/outputs/externally_anchored_big_five/externally_anchored_big_five_report.md` is the entry point for the geometry-blind IPIP/IPIP-NEO and reviewed-SAPA construction. The mapping was frozen before geometry analysis: strict uses 42 defensible Big Five assignments from `ACCEPT_DIRECT`; extended adds 25 separately flagged `ACCEPT_CLOSE`; expanded includes 109 high-confidence external-taxonomy indicators after all 240 traits were reviewed; the historical hand-predeclared composite is comparator-only. Under strict mapping, Qwen Agreeableness aligns with negative PC3 at direction cosine -0.847133, Pearson r=-0.881714, Spearman rho=-0.837526, and R2=0.777420; the joint-five standardized beta is -0.894047 and held-out partial R2 is 0.524122. The relationship is robust across frozen constructions but not a universal PC3 identity: Llama Agreeableness instead aligns with PC2, and Gemma shows a moderate PC3 relation. Use `artifact_inventory.csv` for all hashes/URLs and `big_five_viewer_integration_report.md` for viewer QA/publication status. This remains same-space activation-derived evidence, not independent human psychometric validation.
...
Multimodel grouped trait landscape: `research/outputs/persona_trait_surface_viewer/persona_trait_surface_viewer.html` switches without reload among Qwen, Llama, and Gemma and between the original five editorial groups and the five frozen human-anchored strict Big Five profiles. Editorial remains the default; each model/profile owns its coordinates, exact nodes, surfaces, support masks, flat planes, and fit diagnostics. The original 1,375 group rows/model remain unchanged; Big Five adds 1,375 domain nodes/model in `big_five_surface_node_scores.csv` and separate meshes in `big_five_trait_surface_data.json`. Camera and persona-name selection persist across switches. Qwen editorial science reproduces exactly. Data/DOM checks and actual local Chrome 152/Plotly WebGL verification passed. Open the completed HTML, not `viewer_template.html`; use `trait_surface_methodology.md` and `research/outputs/externally_anchored_big_five/big_five_viewer_integration_report.md` for methods, verification, and publication status.

Multimodel PC-ranked trait profiles: `research/outputs/persona_trait_ridge_plots/persona_trait_ridges.html` switches without reload among Qwen, Llama, and Gemma, with all 275 personas ordered by each selected model's own PC1/PC2/PC3 coordinates. The unchanged 15-trait editorial mode remains default. Big Five mode adds five composite profiles and a selector for strict, extended, external-taxonomy expanded, and historical constructions, with raw projections and constituent facets visible. The 12,375-row editorial bundle remains exact; Big Five data come from the frozen audit bundle. Qwen uses canonical geometry; Llama/Gemma use model-local PCA with the established multimodel sign orientation. Static checks and actual local Chrome 152 verification passed, including real score replacement and persona persistence. Same-space activation evidence, not independent psychological validation.
