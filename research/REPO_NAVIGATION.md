...
Updated UTC: 2026-09-13
Last commit when updated: 88ee7ea (base before comparison-viewer update)
...
Held-out trait-profile geometry equalizer: `research/outputs/trait_profile_geometry_explorer/trait_profile_geometry_explorer.html` is the active companion for interactively editing all 240 trait percentiles while viewing the 275 canonical Qwen personas in 2D/3D PC space. Beneath the scatter, it can compare the first editable profile with a second saved persona as two percentile lines and a zero-centered first-minus-second difference chart, with a 2D/3D actual-to-actual connector. Its optional prominence sort reorders all three charts and sliders together by current edited percentile without changing predictor feature order. For each selected persona it uses a transparent Ridge bundle fitted and tuned without that persona, reproduces the saved LOPO baseline, and continues through the same holdout model after edits. It shows actual/predicted points, displacement, q95 empirical held-out error references, nearest profiles, and the canonical OOD heuristic. Use `trait_profile_geometry_implementation_report.md` and `trait_profile_geometry_methodology.md` for scope, `run_trait_profile_geometry_explorer.py` for generation, and `artifact_inventory.csv` for hashes/URLs. This is a predicted counterfactual viewer, not new observed behavior, causal trait evidence, or human personality measurement. The canonical `research/visualizations/persona_geometry_explorer.html` remains byte-identical.
...
Trait-profile geometry equalizer: `research/outputs/trait_profile_geometry_explorer/trait_profile_geometry_explorer.html` preserves the canonical 275-persona 2D/3D distribution in an active source-built companion and adds a complete 240-trait percentile equalizer. Selected-persona predictions are genuinely persona-held-out, edits remain on that held-out model, and the interface shows q95 LOPO error references plus the deployed profile-manifold OOD context. Python, Node, and live headless-Chrome checks pass, including all 275 baseline predictions and 30 deterministic modified profiles. Open the completed HTML, not `viewer_template.html`.
