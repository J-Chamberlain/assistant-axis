...
The completed aggregate Rosetta follow-up is registered under `research/outputs/human_model_rosetta_translation/`. Start with its report, freeze, source provenance, and verification artifacts. It recovers prior AA-12 correspondence and finds no held-out low-complexity translation improvement beyond identity for Qwen; classification is WEAK / ABSENT. This is aggregate profile-shape evidence only and does not modify or explain V1.

The Rosetta profile-visual layer is under `research/outputs/human_model_rosetta_translation/profile_visuals/`. Start with `human_model_trait_profile_comparison.html`, `trait_profile_visual_comparison_report.md`, `cluster_pair_metrics_12.csv`, `cluster_pair_metrics_45.csv`, and `trait_agreement_summary_45.csv`. It compares the same 24 fixed anchor-only Rosetta pairs at 12- and 45-trait resolutions; all visuals are descriptive and preserve the WEAK / ABSENT translation classification.
