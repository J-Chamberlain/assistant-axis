...
**Current integration update (2026-09-12, AA-7):** Completed the preregistered two-phase human-supported trait convergence and aligned-subspace study under `research/outputs/human_supported_trait_convergence/`. The exact AA-1 set was frozen before outcome analysis. In the primary fixed-split test, the human-supported 12 beat 0/6 matched random-real k=12 controls and only 2/6 fold-local persona-span controls (Qwen and Gemma core); evidence that human psychometric defensibility selects unusually efficient compact model coordinates is therefore **WEAK / ABSENT**. The set beat isotropic directions in 6/6 comparisons, but geometry-optimized 12-trait sets were better in every model/scope and PC4-PC6 were harder than PC1-PC3. Separately, held-out PC1-PC6 role-score alignment was strong (mean coordinate r=0.879 Llama→Qwen, 0.809 Gemma→Qwen, 0.908 Gemma→Llama; all permutation p=0.001). Eleven frozen traits had high and one moderate aligned recurrence; all five strict Big Five directions had high recurrence. Agreeableness was preregisteredly reconciled after alignment (pairwise cosines 0.900/0.952/0.938), and adding PC4-PC6 materially improved human-12, Big Five, and Agreeableness direction recurrence. This is model-only representational convergence over shared roles and activation-derived trait directions, not human/model equivalence.
