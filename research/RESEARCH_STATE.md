...
**Current update (2026-09-15):** Completed the isolated aggregate human↔model Rosetta profile-translation follow-up under `research/outputs/human_model_rosetta_translation/`. The prior AA-12 cluster correspondence was recovered from frozen local artifacts. A 12-trait predeclared anchor-only matching design evaluated the exact 33-trait complement with Qwen primary, fixed-pair Llama/Gemma summaries, 24 held-out matched-pair folds, and three 2,000-draw structural nulls. Identity outperformed every learned low-complexity candidate; all nulls were non-supportive. Classification is WEAK / ABSENT for an aggregate Rosetta translation. This result is descriptive aggregate profile evidence only, does not modify or explain V1, and does not generalize to individual people or external cohorts.

**Current visualization update (2026-09-15):** Added descriptive 12-trait/45-trait human-Qwen profile comparison visuals under `research/outputs/human_model_rosetta_translation/profile_visuals/`. The same 24 fixed Rosetta pairings are shown as overlays, residual heatmaps, 45-trait dumbbells, pair-metric tables, trait-agreement summaries, and a self-contained HTML explorer. The 12-trait profiles are descriptively cleaner on average than the 45-trait profiles, but the visualization does not alter the WEAK / ABSENT translation result or support individual-level transfer.
