...
**Current integration update (2026-09-12, AA-7):** Completed the preregistered two-phase human-supported trait convergence and aligned-subspace study under `research/outputs/human_supported_trait_convergence/`. The exact AA-1 set was frozen before outcome analysis. In the primary fixed-split test, random-real one-sided p-values were 0.2395/0.2615 Qwen, 0.7405/0.3533 Llama, and 0.1497/0.1257 Gemma for core/extended: the human-supported 12 beat 0/6 matched random-real k=12 controls. Only the Qwen core (p=0.0220) and Gemma core (p=0.0279) fold-local persona-span controls passed, so evidence that human psychometric defensibility selects unusually efficient compact model coordinates is **WEAK / ABSENT**. The set beat isotropic directions in 6/6 comparisons, but isotropic ambient directions are a weaker generic control; geometry-optimized 12-trait sets were better in every model/scope and PC4-PC6 were harder than PC1-PC3. Separately, held-out PC1-PC6 role-score alignment was strong (mean coordinate r=0.879 Llama→Qwen, 0.809 Gemma→Qwen, 0.908 Gemma→Llama; all permutation p=0.001). Eleven frozen traits had high and one moderate aligned recurrence; all five strict Big Five directions had high recurrence. Agreeableness was preregisteredly reconciled after alignment (pairwise cosines 0.900/0.952/0.938), and adding PC4-PC6 materially improved human-12, Big Five, and Agreeableness direction recurrence. This is model-only representational convergence over shared roles and activation-derived trait directions, not human/model equivalence. The separately completed Qwen-first human-construct branch and the newer correlation-defined family inventory did not use AA-7 results for construct selection.

**Current AA-1 source update (2026-09-12, Qwen-first human-construct bridge):** Preserved the exact user-supplied Claude Opus 5 blinded judgments, measured agreement against the frozen Codex review, resolved eight wording/direction flags against primary SAPA/IPIP/SPI sources, built a sourced 126-construct human library, and froze 37 Qwen-only broad-domain/aspect/facet hypotheses. This source work is discovery-only; it used no AA-7, Llama, or Gemma result to choose constructs and projected no respondent.

**Current AA-1 inventory update (2026-09-12):** `research/outputs/qwen_trait_family_human_inventory/` now provides the requested simpler intermediate object. All 1,440 Qwen PC1-PC6 x trait associations are saved. The family rule was frozen before interpretation: `|r| >= 0.50`, PC-local BH-FDR q below 0.01, and at least 95% sign stability over 2,000 fixed-seed role bootstraps. It selects 328 signed memberships spanning 238 traits, with family sizes PC1 47/123, PC2 31/44, PC3 42/31, PC4 2/6, PC5 1/0, and PC6 0/1 for positive/negative poles. A neutral-ID packet was committed before matching to the full 126-construct SAPA library. The final 48-row inventory contains 37 unique construct/item candidates and 350 unique SAPA items; at family level 4 have a strong available match, 5 partial, 1 facet/item-only, and 2 empty poles no match. Verification passes 34 checks. This is the mandated stopping point; no respondent was scored or projected, no cross-model or AA-7 result was used, no correspondence test ran, and no next experiment was selected.

**Current AA-1 specificity refinement (2026-09-12):** `research/outputs/qwen_pc_trait_specificity/` preserves the 328 association-defined memberships and adds the intended second criterion: paired-bootstrap preference for the target PC over PC1-PC6. The primary rule was committed before results. Requiring target-is-largest probability at least 0.95 contracts the inventory to 202 PC-defining memberships, 70 with observed PC1-PC6 concentration at least 0.75. Signed associated/defining counts are PC1 47/35 and 123/95, PC2 31/12 and 44/23, PC3 42/18 and 31/18, PC4 2/0 and 6/1, PC5 1/0 and 0/0, and PC6 0/0 and 1/0. The exact Qwen Affiliation surface was recovered as the percentile mean of `empathetic`, `agreeable`, and `altruistic`; it has PC3 r=-0.944857, largest off-target PC1 r=-0.110061, margin 0.834796, and concentration 0.978101. The 48 frozen SAPA judgments are unchanged and now show defining/high/Pareto coverage. Verification passes 29 checks. No respondent, Llama/Gemma result, AA-7 result, correspondence test, or next-experiment choice entered the workflow.

**Observed:** PC1-PC3 are Qwen's strict individually stable core; PC4-PC6 retain weaker reproducible structure; negative Qwen PC3 strongly aligns with frozen Agreeableness directions; compact optimized real traits outperform matched generic directions; the independently human-supported 12 do not outperform matched random-real traits; model-local PC1-PC6 role-score subspaces align above permutation and activation-derived human-supported/Big Five directions recur after alignment; full 240-trait prediction has near-complete same-space basis coverage; the correlation-defined signed Qwen families contain 328 memberships, of which 202 are preferentially PC-defining and 70 highly concentrated within PC1-PC6, and have documented SAPA availability through 37 construct/item candidates and 350 items; 12 SAPA links have moderate-or-better human structural support; and occupation-level human personality signal is small relative to within-occupation variation.

**Interpretation:** The defensible integrated model is layered and provenance-aware: Qwen PC3 includes but is broader than Agreeableness, secondary PCA coordinates are useful without equal-status labels, compact geometry-optimized traits can be semantically efficient without making human-supported labels unusually efficient or the full bank independently psychological, and cross-model aligned recurrence supports shared model role-space organization without establishing model/human correspondence. Association-defined families are broad; the preferentially specific core is more diagnostic, with PC1-PC3 retaining substantial cores and PC4-PC6 nearly empty under the frozen dominance rule. The frozen SAPA coverage of that core is qualitatively more focused and uneven, not evidence of equivalence.

**Hypotheses:** Shared role instructions and/or shared representational organization may explain why activation-derived trait directions recur after rotational alignment. Proposed SAPA family matches may or may not measure a shared latent property. Neither AA-7 nor the inventory distinguishes these sources or identifies causal latent mechanisms.

**Unknown:** Independent experts may further contract the SAPA bridge; actual quantitative human/model correspondence, respondent-level recoverability under planned missingness, causal relations, and cross-model generalization of the correlation-defined families remain untested. AA-7's preregistered gate for designing a human-respondent projection study did not pass.
...
### Current Session: Qwen PC-Associated Versus PC-Defining Trait Specificity (2026-09-12)

Completed the CPU-only refinement under `research/outputs/qwen_pc_trait_specificity/` from starting/source SHA `667acdd634bfb8c39d40432fe60f0c5ef3bbbeeb`. The primary rule was frozen at `595c1e2` before specificity results were computed: unchanged AA-1 association plus target-is-largest probability at least 0.95 over 2,000 paired role bootstraps. All 1,440 trait-target rows retain PC1-PC6 correlations and bootstrap strength/specificity distributions; the 328 associated memberships contract to 202 PC-defining and 70 highly concentrated memberships. All-trait Pareto fronts, the complete 3 x 3 sensitivity grid, secondary top-three composites, the exact canonical PC3 Affiliation composite audit, the unchanged 48-row SAPA mapping with diagnostic-core intersections, and 29-check verification are saved.

Immediate continuity: inspect `qwen_pc_specificity_report.md`, `qwen_pc_defining_trait_sets.csv`, and `qwen_pc_specificity_human_summary.csv`. This thread stops at inspection of the refined inventories; no next experiment is selected. Last committed checkpoint is `595c1e2` on `codex/aa1-qwen-pc-trait-specificity`; the final result/maintenance commit follows this state update. Claims Register and sticky notes remain unchanged.

### Current Session: Correlation-Defined Qwen Trait Families → SAPA Inventory (2026-09-12)

Completed the CPU-only inventory under `research/outputs/qwen_trait_family_human_inventory/` from validated AA-1 source commit `313c5cff6e071d707d37b1318343cde4ed510725`. The numerical rule, blind packet, semantic judgments, and assembled inventory were frozen in that order. All 1,440 associations, 328 selected memberships, twelve signed family summaries, 48 SAPA mapping rows, exhaustive item wording, source hashes, and a 34-check deterministic verification are saved. The inventory uses the full 126-construct library and represents 350 distinct SAPA items. The exact stopping point was honored: no respondent scoring/projection, Llama/Gemma comparison, AA-7 input, human/model test, or next-experiment selection.

Immediate continuity: inspect `qwen_pc_trait_family_human_inventory.csv` and its readable report. No subsequent experiment is selected in this workflow. Last result commit before canonical-state reconciliation: `4d2bdb4` on `codex/aa1-qwen-trait-family-human-inventory`. Claims Register and sticky notes are unchanged by this descriptive inventory.
