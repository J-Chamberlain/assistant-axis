...
# AA-10 session update (2026-09-15): Completed human-only SAPA HiFWB reproducibility under `research/outputs/sapa_hifwb_reproducibility/`. The preserved exploratory 13-item common-factor diagnostics reproduce exactly; the historical Big Five OOF result is reproduced to target under a newly frozen deterministic five-fold seed because the original fold assignment is unknown. Frozen non-affect, content-balanced (>=2 and >=3 contents), vitality-auxiliary, leave-one-content-out, and fold-safe PC1-loading-weighted sensitivities were run. Out-of-fold full five-coordinate Ridge and RBF Nyström models outperform the reconstructed frozen backbone cubic (Ridge ΔR²=0.1017, bootstrap 95% CI [0.0867, 0.1176]). This is predictive/cross-sectional only; the score is not a validated HiFWB scale, and no model geometry or respondent-level data were committed.
