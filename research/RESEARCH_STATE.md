...
### Current Session: Qwen Trait Sparsity and Basis-Coverage Audit (2026-09-11)

Completed `research/outputs/qwen_trait_sparsity_prediction/` with exact full-model reproduction, fixed editorial-15 benchmarking, all-trait marginal and conditional rankings, leakage-safe joint and PC-specific nested forward selection, sparse-model stability, redundancy groups, target permutations, random real-trait subsets, target-independent isotropic activation directions, fold-local training-persona-span random directions, direct trait-span PC-loading coverage, and role-norm auditing. Selected real traits provide a large compact-k advantage, but the 240-direction persona-span control nearly reaches full-trait performance; the scientific conclusion is a mixture rather than a pure semantic-trait or pure ambient-random-basis result. All verification checks pass, including byte-identical rerun of the primary deterministic paths.

Next step: preregister a frozen compact/profile prediction for a genuinely new Qwen persona and test it against newly elicited activation coordinates without refitting. Last commit before result registration: `afb53d4`, branch `codex/qwen-trait-sparsity`. Claim 21 remains Observed but now records the compact advantage, random-basis controls, direct span coverage, redundancy, and stronger same-space caveat.
