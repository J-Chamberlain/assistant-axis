...
**Current session update (2026-09-11, AA-5 NLSY97 occupational personality stability):** Completed the human-only audit under `research/outputs/nlsy97_occupation_personality_stability/` after freezing the specification at commit `13fb08f` before outcome inspection. The official NLS Investigator tagset, codebook, and 8,984-row extract were reacquired after concurrent worktree cleanup removed the prior gitignored copy; all three key SHA-256 hashes exactly match AA-1. The primary cohort reproduces 7,044 complete Round-12 TIPI respondents and 6,261 with valid same-wave occupation, using the Round-12 cumulative-case weight plus VSTRAT/VPSU. Frozen joint criteria yield 9 strong and 18 moderate/exploratory reportable narrow occupations, 25 strong and 13 moderate intermediate groups, and 18 strong plus 2 moderate broad families. Empirical five-dimensional error is roughly 0.43-0.46 at N=20, 0.26-0.28 at N=50, and 0.17-0.18 at N=100. Survey-weighted sampling-error-corrected ICCs are only 0.038-0.078 at the narrow level and 0.008-0.032 at the broad level, and fixed classification is near chance, so stable aggregate means coexist with dominant within-occupation heterogeneity and weak occupational distinctiveness. Round-6/Round-12 recurrence is mixed and more consistent for Conscientiousness. Secretary/administrative assistant is a strong narrow future candidate; accountant/auditor, caregiver, and the artisan proxy are moderate; counselor and several sparse professional/creative occupations require semantically broader parents; lawyer has no stable reportable parent. All 19 verification checks pass and a complete rerun exactly reproduces 15 generated artifact hashes. No model geometry, model inference, human-to-model projection, GPU, RunPod, activation extraction, or external model API was used; no respondent rows were committed. Next step, if pursued, is human review of the candidate list and a separate preregistration that freezes construct correspondence and comparison metrics before any model data are loaded.
