...
**Current AA-1 axis-specificity update (2026-09-12):** `research/outputs/qwen_trait_axis_specificity/` now distinguishes association strength from PC1-PC6 specificity without recomputing or changing the prior 328-member family inventory. The frozen primary layer requires the target to be the strict largest absolute correlation, bootstrap target-dominance probability at least .95 over 2,000 fixed-seed role resamples, and six-PC purity at least .70. It retains 88/328 memberships (26.8%): PC1 positive/negative 12/48, PC2 3/10, PC3 7/8, and PC4-PC6 zero on both poles. The original rows classify as 88 axis-specific, 150 target-dominant-but-diffuse, and 90 non-target-dominant. The documented Affiliation composite is strongly PC3-specific (`r=-0.970129`, purity `0.965818`); the other four editorial composites are less pure. A new neutral-ID remapping yields 31 rows, 22 unique construct/item candidates, and 195 SAPA items; three signed families have a strong available match, three partial-only, and six have no match because their primary marker set is empty. Verification passes 54 checks including a deterministic rerun. This is another mandated stopping point: no respondent scoring/projection, cross-model or AA-7 result, correspondence test, or next-experiment selection entered the workflow.

**Observed:** PC1-PC3 are Qwen's strict individually stable core; PC4-PC6 retain weaker reproducible structure; negative Qwen PC3 strongly aligns with frozen Agreeableness directions; compact optimized real traits outperform matched generic directions; the independently human-supported 12 do not outperform matched random-real traits; model-local PC1-PC6 role-score subspaces align above permutation and activation-derived human-supported/Big Five directions recur after alignment; full 240-trait prediction has near-complete same-space basis coverage; the correlation-defined signed Qwen families contain 328 memberships and have documented SAPA availability through 37 construct/item candidates and 350 items; the stricter axis-specificity layer retains 88 memberships and its filtered inventory contains 22 construct/item candidates and 195 items; 12 SAPA links have moderate-or-better human structural support; and occupation-level human personality signal is small relative to within-occupation variation.

**Interpretation:** The defensible integrated model is layered and provenance-aware: Qwen PC3 includes but is broader than Agreeableness, secondary PCA coordinates are useful without equal-status labels, compact geometry-optimized traits can be semantically efficient without making human-supported labels unusually efficient or the full bank independently psychological, and cross-model aligned recurrence supports shared model role-space organization without establishing model/human correspondence. The signed-family and stricter axis-specificity inventories describe available SAPA coverage at two distinct filtering levels; neither validates equivalence.
