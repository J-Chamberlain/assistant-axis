...
Descriptive profile visuals for that study are under `research/outputs/human_model_rosetta_translation/profile_visuals/`. They preserve the 24 fixed Rosetta pairings and compare the exact 12-trait anchor view with the full 45-trait direct bridge using common canonical score scales, static heatmaps/overlays/dumbbells, trait-agreement tables, and a self-contained HTML explorer. The 12-trait profiles are descriptively cleaner on average, but this does not change the WEAK / ABSENT held-out translation result.
