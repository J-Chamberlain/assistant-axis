...
- A first Qwen-only PC1 x PC2 trait-region overlay shows that PC1-band-relative trait labels materially differ from global labels (mean top-3 overlap 0.18), making PC2 easier to inspect locally without broad PC1 gradients dominating the annotations.
...
14. Use `research/outputs/recovered_role_cloud_analysis/` for prior adaptive extraction comparisons; do not rerun GPU solely to recover hook-based trickster/editor vectors. If procedural-professional extraction remains important, redesign the role target/anchoring instead of repeating editor unchanged.
