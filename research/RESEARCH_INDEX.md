...
Grouped 3D companion: `research/outputs/persona_trait_surface_viewer/persona_trait_surface_viewer.html` offers the same five groups as a slider over any two PCs, with height equal to the mean of three original trait percentiles. Stronger fixed-range color, fabric-only mode and synchronized yaw/pitch/roll/zoom sliders with numeric entry and view presets address manual inspection requests. Complete group/member scores, fit diagnostics, provenance and tests accompany it. No existing viewer overwritten. Data and control-state tests pass; local browser automation was blocked, so interactive WebGL verification remains user-side. See `trait_surface_methodology.md` for averaging and same-space evidence caveats.
