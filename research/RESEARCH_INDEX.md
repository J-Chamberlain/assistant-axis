...
The human-only SAPA HiFWB continuation is under `research/outputs/sapa_hifwb_reproducibility/`. The preserved exploratory target reproduces exactly for the 13-item common-factor diagnostics and nearly exactly for Big Five OOF prediction under a newly frozen deterministic five-fold assignment. Content-balanced, non-affect, leave-one-content-out, and fold-safe loading sensitivities are reported. The out-of-fold five-coordinate models outperform the reconstructed one-dimensional frozen backbone; this is predictive and cross-sectional, not causal or a validated HiFWB scale.
# AA-13 (2026-09-16): broad SAPA PCA package is under `research/outputs/broad_sapa_pca_wellbeing/`; external SAPA V5 input and frozen-score reconstruction are documented there.
# AA-13 follow-up artifacts (2026-09-16): parallel analysis, stability, Big Five alignment, and incremental bootstrap results are in `research/outputs/broad_sapa_pca_wellbeing/`.
