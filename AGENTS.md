...
Use role-based model selection rather than a fixed model name:

- Use GPT-6 Astra for research design, scientific synthesis, ambiguous analytical decisions, and complex end-to-end investigations.
- Use Sol or Terra when the analytical method is frozen and work is primarily implementation, computation, and verification.
- Use Luna for mechanical extraction, formatting, indexing, and routine edits.
- Cards should recommend the least expensive model and reasoning setting that can be trusted for the task.
- Always record the model that actually performed the work, including its reasoning setting when available. Never inherit a model name from a template or placeholder.
- If actual model identity is unavailable, record it as unknown rather than inferring it.
- Explicit user model instructions supersede the default recommendation.

The fixed GPT-5.5 instruction introduced on May 24, 2026 corrected inaccurate provenance metadata; it was not a durable comparison against later models. This role-based policy supersedes that instruction at the user's explicit request.
